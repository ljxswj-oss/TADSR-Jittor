...
from jittor_tadsr_full.unet_2d_condition import UNetResnetBlock2DTester, TADSRUNetUpBlock1Resnet0Tester
...
from unet_upblock0_upsampler_common import metadata as upblock0_metadata

ORACLE = OUT / 'oracle_tensors_unet_upblock1_resnet0'
META = ORACLE / 'unet_upblock1_resnet0_oracle_metadata.json'
UP1_RESNET0_WEIGHTS = OUT / 'converted_unet_upblock1_resnet0_effective_weights.npz'
...
def load_oracle():
    ...
    if missing:
        return None, {
            'status': 'BLOCKED',
            'reason': 'Run python3 tools/export_tadsr_unet_upblock1_resnet0_oracle.py first.',
            'missing': missing,
        }
    tensors = {p.stem: np.load(p).astype(np.float32) for p in ORACLE.glob('*.npy')}
    if not tensors:
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_upblock1_resnet0_oracle.py first.'}
    return tensors, None


def metadata():
    meta = upblock0_metadata()
    meta['upblock1_resnet0_metadata'] = load_json(META)
    return meta


def resnet0_tester():
    meta = load_json(META)
    return UNetResnetBlock2DTester(UP1_RESNET0_WEIGHTS, meta.get('resnet0_config', {}).get('prefix', 'up_blocks_1_resnets_0'), meta)


def bridge_tester():
    return TADSRUNetUpBlock1Resnet0Tester(
        ENTRY_WEIGHTS,
        DOWN0_RESNET0_WEIGHTS,
        DOWN0_ATTENTION0_WEIGHTS,
        DOWN0_RESNET1_WEIGHTS,
