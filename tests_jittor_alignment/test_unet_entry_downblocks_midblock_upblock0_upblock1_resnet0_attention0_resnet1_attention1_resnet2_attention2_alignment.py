...
def main() -> int:
    ...
    name = 'jittor_unet_entry_downblocks_midblock_upblock0_upblock1_resnet0_attention0_resnet1_attention1_resnet2_attention2_alignment'
    title = 'TADSR UNet Entry -> DownBlocks -> MidBlock -> UpBlock0 -> UpBlock1 ResNet0 -> Attention0 -> ResNet1 -> Attention1 -> ResNet2 -> Attention2 Bridge Alignment'
    marker = 'TADSR_UNET_ENTRY_DOWNBLOCKS_MIDBLOCK_UPBLOCK0_UPBLOCK1_RESNET0_ATTENTION0_RESNET1_ATTENTION1_RESNET2_ATTENTION2_ALIGNMENT'
    if blocked:
        result = blocked_result(name, title, blocked)
        print(f'{marker}:', result['status'])
        return 0 if result['status'] == 'PASS' else 1
    tester = bridge_tester()
    got = tester.run_entry_downblocks_midblock_upblock0_upblock1_resnet0_attention0_resnet1_attention1_resnet2_attention2(
