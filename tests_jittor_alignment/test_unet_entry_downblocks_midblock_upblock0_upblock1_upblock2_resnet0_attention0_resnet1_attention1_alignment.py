...
from unet_upblock2_attention1_common import (
    add_metrics,
    blocked_result,
    bridge_tester,
    load_oracle,
...
def main() -> int:
    ...
    name = 'jittor_unet_entry_downblocks_midblock_upblock0_upblock1_upblock2_resnet0_attention0_resnet1_attention1_alignment'
    title = 'TADSR UNet Entry -> DownBlocks -> MidBlock -> UpBlock0 -> UpBlock1 -> UpBlock2 Resnet0 -> Attention0 -> Resnet1 -> Attention1 Bridge Alignment'
    if blocked:
        result = blocked_result(name, title, blocked)
        print('TADSR_UNET_ENTRY_DOWNBLOCKS_MIDBLOCK_UPBLOCK0_UPBLOCK1_UPBLOCK2_RESNET0_ATTENTION0_RESNET1_ATTENTION1_ALIGNMENT:', result['status'])
        return 0 if result['status'] == 'PASS' else 1

    tester = bridge_tester()
    got = tester.run_entry_downblocks_midblock_upblock0_upblock1_upblock2_resnet0_attention0_resnet1_attention1(
    ...
    expected = {
        ...
        'upblock2_attention1_input': 'entry_upblock2_attention1_input',
        'upblock2_attention1_output': 'entry_upblock2_attention1_output',
    }

    diag = {}
    for stage, tensor_name in expected.items():
        if stage in {'conv_in', 'time_embedding'}:
            tol = 1e-4
        elif stage in {'upblock2_attention0_output', 'upblock2_resnet1_output', 'upblock2_attention1_output'}:
            tol = 2e-3
        else:
            tol = 2e-3
    ...
    result = {
        ...
        'note': (
            ...
            '-> up_blocks.2.attentions.0 -> up_blocks.2.resnets.1 -> up_blocks.2.attentions.1. '
            'It deliberately stops before up_blocks.2.resnets.2 '
            'and does not mark full up_blocks.2 complete.'
        ),
    }
    write_report(name, title, result)
    print('TADSR_UNET_ENTRY_DOWNBLOCKS_MIDBLOCK_UPBLOCK0_UPBLOCK1_UPBLOCK2_RESNET0_ATTENTION0_RESNET1_ATTENTION1_ALIGNMENT:', status)
