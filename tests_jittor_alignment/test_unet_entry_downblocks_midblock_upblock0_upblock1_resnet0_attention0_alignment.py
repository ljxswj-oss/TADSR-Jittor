...
def main() -> int:
    ...
    name = 'jittor_unet_entry_downblocks_midblock_upblock0_upblock1_resnet0_attention0_alignment'
    title = 'TADSR UNet Entry -> DownBlocks -> MidBlock -> UpBlock0 -> UpBlock1 ResNet0 -> Attention0 Bridge Alignment'
    if blocked:
        result = blocked_result(name, title, blocked)
        print('TADSR_UNET_UPBLOCK1_ATTENTION0_BRIDGE_ALIGNMENT:', result['status'])
        return 0 if result['status'] == 'PASS' else 1
    tester = bridge_tester()
    got = tester.run_entry_downblocks_midblock_upblock0_upblock1_resnet0_attention0(
    ...
    expected = {
        ...
        'upblock1_attention0_input': 'entry_upblock1_attention0_input',
        'upblock1_attention0_output': 'entry_upblock1_attention0_output',
    }
    diag = {}
    for stage, path in expected.items():
        tol = 1e-4 if stage in {'conv_in', 'time_embedding', 'upblock1_resnet0_concat_input', 'upblock1_attention0_input'} else 2e-3
        diag[stage] = add_metrics(got[stage], tensors[path], tolerance=tol)
    status = status_from_diagnostics(diag)
    result = {
        'status': status,
        'diagnostics': diag,
        'note': 'Bridge validates entry -> all down_blocks -> full local mid_block -> full local up_blocks.0 -> up_blocks.1.resnets.0 -> up_blocks.1.attentions.0 only. It still does not execute up_blocks.1.resnets.1, full up_blocks.1, or full UNet.',
    }
    write_report(name, title, result)
    print('TADSR_UNET_UPBLOCK1_ATTENTION0_BRIDGE_ALIGNMENT:', status)
