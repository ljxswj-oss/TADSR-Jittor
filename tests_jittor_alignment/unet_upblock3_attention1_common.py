...
from jittor_tadsr_full.unet_2d_condition import UNetAttention0Transformer2DTester, TADSRUNetUpBlock3Attention1Tester
...
from unet_upblock3_resnet1_common import metadata as upblock3_resnet1_metadata

ORACLE = OUT / 'oracle_tensors_unet_upblock3_attention1'
META = ORACLE / 'unet_upblock3_attention1_oracle_metadata.json'
...
def load_oracle():
    ...
    if missing:
        return None, {
            'status': 'BLOCKED',
            'reason': 'Run python3 tools/export_tadsr_unet_upblock3_attention1_oracle.py first.',
            'missing': missing,
        }
    tensors = {p.stem: np.load(p).astype(np.float32) for p in ORACLE.glob('*.npy')}
    if not tensors:
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_upblock3_attention1_oracle.py first.'}
    return tensors, None


def metadata():
    meta = upblock3_resnet1_metadata()
    meta['upblock3_attention1_metadata'] = load_json(META)
    return meta


def attention1_tester():
    return UNetAttention0Transformer2DTester(UP3_ATTENTION1_WEIGHTS, metadata=load_json(META))


def bridge_tester():
    return TADSRUNetUpBlock3Attention1Tester(
        ENTRY_WEIGHTS,
        DOWN0_RESNET0_WEIGHTS,
        DOWN0_ATTENTION0_WEIGHTS,
        DOWN0_RESNET1_WEIGHTS,
