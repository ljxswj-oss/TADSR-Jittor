...
def main() -> int:
    ...
    if blocked:
        result = blocked
        write_report('jittor_encoder_to_decoder_upblock3_alignment', 'TimeAware VAE Encoder To Decoder UpBlock3 Alignment', result)
        print(f"TIME_VAE_ENCODER_TO_DECODER_UPBLOCK3_ALIGNMENT: {result['status']}")
        return 1

    tester = TimeVAEBlockTester(WEIGHTS)
    got = tester.run_encoder_stage0123_mid_tail_quant_to_decoder_upblock3(
    ...
    diagnostics = {
        ...
        'decoder_upblock3': add_metrics(got['decoder_upblock3'], oracle['real_decoder_upblock3_output']),
    }
    result = {
        'status': status_from_diagnostics(diagnostics),
        'target': 'encoder image path -> quant_conv -> decoder up_blocks.3',
