...
from jittor_tadsr_full.unet_2d_condition import TADSRUNetUpBlock1LocalTester
...
ORACLE = OUT / 'oracle_tensors_unet_upblock1_local'
META = ORACLE / 'unet_upblock1_local_oracle_metadata.json'
...
def load_oracle():
    ...
    if missing:
        return None, {
            'status': 'BLOCKED',
            'reason': 'Run python3 tools/export_tadsr_unet_upblock1_local_oracle.py first.',
            'missing': missing,
        }
    tensors = {p.stem: np.load(p).astype(np.float32) for p in ORACLE.glob('*.npy')}
    if not tensors:
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_upblock1_local_oracle.py first.'}
    return tensors, None


def metadata():
    meta = upblock1_upsampler_metadata()
    meta['upblock1_local_metadata'] = load_json(META)
    return meta


def local_tester():
    return TADSRUNetUpBlock1LocalTester(
        ENTRY_WEIGHTS,
        DOWN0_RESNET0_WEIGHTS,
        DOWN0_ATTENTION0_WEIGHTS,
        DOWN0_RESNET1_WEIGHTS,
