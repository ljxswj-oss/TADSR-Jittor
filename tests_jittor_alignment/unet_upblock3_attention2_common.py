...
from jittor_tadsr_full.unet_2d_condition import UNetAttention0Transformer2DTester, TADSRUNetUpBlock3Attention2Tester
from unet_upblock3_resnet2_common import *  # noqa: F401,F403
from unet_upblock3_resnet2_common import load_oracle as _load_resnet2_oracle
from unet_upblock3_resnet2_common import metadata as upblock3_resnet2_metadata

ORACLE = OUT / 'oracle_tensors_unet_upblock3_attention2'
META = ORACLE / 'unet_upblock3_attention2_oracle_metadata.json'
UP3_ATTENTION2_WEIGHTS = OUT / 'converted_unet_upblock3_attention2_effective_weights.npz'


def load_oracle():
    _prev, prev_blocked = _load_resnet2_oracle()
    if prev_blocked:
        return None, prev_blocked
    required = [ORACLE, META, UP3_ATTENTION2_WEIGHTS]
    missing = [str(p) for p in required if not p.exists()]
    if missing:
        return None, {
            'status': 'BLOCKED',
            'reason': 'Run python3 tools/export_tadsr_unet_upblock3_attention2_oracle.py first.',
            'missing': missing,
        }
    tensors = {p.stem: np.load(p).astype(np.float32) for p in ORACLE.glob('*.npy')}
    if not tensors:
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_upblock3_attention2_oracle.py first.'}
    return tensors, None


def metadata():
    meta = upblock3_resnet2_metadata()
    meta['upblock3_attention2_metadata'] = load_json(META)
    return meta


def attention2_tester():
    return UNetAttention0Transformer2DTester(UP3_ATTENTION2_WEIGHTS, metadata=load_json(META))


def bridge_tester():
    return TADSRUNetUpBlock3Attention2Tester(
        ENTRY_WEIGHTS,
