...
def main() -> int:
    ...
    if blocked:
        result = blocked
        write_report('jittor_decoder_upblocks0123_alignment', 'TimeAware VAE Decoder UpBlocks0-3 Alignment', result)
        print(f"TIME_VAE_DECODER_UPBLOCKS0123_ALIGNMENT: {result['status']}")
        return 1

    tester = TimeVAEBlockTester(WEIGHTS)
    got = tester.run_decoder_upblocks0123(oracle['entry_decoder_midblock_output'], return_intermediates=True)
    diagnostics = {
        ...
        'decoder_upblock3': add_metrics(got['decoder_upblock3'], oracle['entry_decoder_upblock3_output']),
    }
    result = {
        'status': status_from_diagnostics(diagnostics),
        'target': 'decoder.mid_block output -> decoder.up_blocks.0 -> decoder.up_blocks.1 -> decoder.up_blocks.2 -> decoder.up_blocks.3',
