...
def main() -> int:
    ...
    name = 'jittor_unet_downblock2_alignment'
    title = 'TADSR UNet Full Local down_blocks.2 Alignment'
    if blocked:
        result = blocked_result(name, title, blocked)
        print('TADSR_UNET_DOWNBLOCK2_ALIGNMENT:', result['status'])
        return 0 if result['status'] == 'PASS' else 1
    tester = downblock2_tester()
    got = tester.run_downblock2_local(
        tensors['entry_downblock1_output_hidden'],
    ...
    diagnostics = {
        ...
        'downblock2_hidden_states_output': add_metrics(got['downblock2_output'], tensors['local_downblock2_hidden_states_output'], tolerance=2e-3),
    }
    output_state_diag = {}
    for idx, got_state in enumerate(got.get('output_states', [])):
        key = f'local_downblock2_res_sample_{idx}'
        if key in tensors:
    ...
    result = {
        'status': status,
        'target': 'down_blocks.2 local: resnet0 -> attention0 -> resnet1 -> attention1 -> downsampler',
    ...
    print(f"TADSR_UNET_DOWNBLOCK2_OUTPUT_HIDDEN_ALIGNMENT: {status_from_metrics(diagnostics['downblock2_hidden_states_output'])}")
    print(f"TADSR_UNET_DOWNBLOCK2_OUTPUT_STATES_ALIGNMENT: {output_states_status}")
    print(f"TADSR_UNET_DOWNBLOCK2_ALIGNMENT: {status}")
