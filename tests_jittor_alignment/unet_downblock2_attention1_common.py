...
from jittor_tadsr_full.unet_2d_condition import UNetAttention0Transformer2DTester, TADSRUNetDownBlock2Attention1Tester
from jittor_tadsr_full.utils import compare_arrays, write_json

OUT = Path('experiments/full_repro/unet_alignment')
ORACLE = OUT / 'oracle_tensors_unet_downblock2_attention1'
META = ORACLE / 'unet_downblock2_attention1_oracle_metadata.json'
...
def load_oracle():
    required = [
        ...
        DOWN2_RESNET0_WEIGHTS, DOWN2_ATTENTION0_WEIGHTS, DOWN2_RESNET1_WEIGHTS, DOWN2_ATTENTION1_WEIGHTS,
    ]
    missing = [str(p) for p in required if not p.exists()]
    if missing:
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_downblock2_attention1_oracle.py first.', 'missing': missing}
    tensors = {p.stem: np.load(p).astype(np.float32) for p in ORACLE.glob('*.npy')}
    if not tensors:
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_downblock2_attention1_oracle.py first.'}
    return tensors, None
...
def attention1_tester():
    return UNetAttention0Transformer2DTester(DOWN2_ATTENTION1_WEIGHTS, metadata=load_json(META))


def bridge_tester():
    return TADSRUNetDownBlock2Attention1Tester(
        ENTRY_WEIGHTS,
