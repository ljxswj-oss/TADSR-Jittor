...
from jittor_tadsr_full.unet_2d_condition import TADSRUNetDownBlock2DownsamplerTester, TADSRUNetDownBlock2Tester
from jittor_tadsr_full.utils import compare_arrays, write_json

OUT = Path('experiments/full_repro/unet_alignment')
ORACLE = OUT / 'oracle_tensors_unet_downblock2_downsampler'
META = ORACLE / 'unet_downblock2_downsampler_oracle_metadata.json'
...
def load_oracle():
    ...
    if missing:
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_downblock2_downsampler_oracle.py first.', 'missing': missing}
    tensors = {p.stem: np.load(p).astype(np.float32) for p in ORACLE.glob('*.npy')}
    if not tensors:
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_downblock2_downsampler_oracle.py first.'}
    return tensors, None
...
def downsampler_tester():
    return TADSRUNetDownBlock2DownsamplerTester(DOWN2_DOWNSAMPLER_WEIGHTS, metadata=load_json(META))


def downblock2_tester():
    return TADSRUNetDownBlock2Tester(
        ENTRY_WEIGHTS,
