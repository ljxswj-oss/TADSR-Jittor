...
def main() -> int:
    ...
    if blocked:
        write_report('jittor_decoder_entry_midblock_upblocks0123_tail_alignment', 'TimeAware VAE Decoder Entry+MidBlock+UpBlocks0-3+Tail Alignment', blocked)
        print(f"TIME_VAE_DECODER_ENTRY_MIDBLOCK_UPBLOCKS0123_TAIL_ALIGNMENT: {blocked['status']}")
        return 1
    tester = TimeVAEBlockTester(WEIGHTS)
    got = tester.run_decoder_entry_midblock_upblocks0123_tail(oracle['entry_synthetic_latent_z'], return_intermediates=True)
    tolerance = 2e-3
    diagnostics = {
    ...
    result = {
        ...
        'target': 'latent z -> post_quant_conv -> decoder.conv_in -> mid_block -> up_blocks.0~3 -> tail',
    ...
    write_report('jittor_decoder_entry_midblock_upblocks0123_tail_alignment', 'TimeAware VAE Decoder Entry+MidBlock+UpBlocks0-3+Tail Alignment', result)
    print(f"TIME_VAE_DECODER_ENTRY_MIDBLOCK_UPBLOCKS0123_TAIL_ALIGNMENT: {result['status']}")
