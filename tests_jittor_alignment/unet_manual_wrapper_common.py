...
from jittor_tadsr_full.unet_2d_condition import TADSRUNetManualWrapperTester
from unet_output_tail_common import *  # noqa: F401,F403
from unet_output_tail_common import load_oracle as _load_output_tail_oracle
from unet_output_tail_common import metadata as _output_tail_metadata

ORACLE = OUT / 'oracle_tensors_unet_manual_wrapper'
META = ORACLE / 'unet_manual_wrapper_oracle_metadata.json'


def load_oracle():
    _prev, blocked = _load_output_tail_oracle()
    if blocked:
        return None, blocked
    ...
    if missing:
        return None, {
            'status': 'BLOCKED',
            'reason': 'Run python3 tools/export_tadsr_unet_manual_wrapper_oracle.py first.',
            'missing': missing,
        }
    tensors = {p.stem: np.load(p).astype(np.float32) for p in ORACLE.glob('*.npy')}
    if not tensors:
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_manual_wrapper_oracle.py first.'}
    return tensors, None


def metadata():
    meta = _output_tail_metadata()
    meta['manual_wrapper_metadata'] = load_json(META)
    return meta


def manual_wrapper_tester():
    return TADSRUNetManualWrapperTester(
        ENTRY_WEIGHTS,
