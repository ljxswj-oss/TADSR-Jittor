...
from unet_downblock3_resnet0_common import add_metrics, blocked_result, bridge_tester, load_oracle, status_from_diagnostics, write_report


def main() -> int:
    tensors, blocked = load_oracle()
    name = 'jittor_unet_entry_downblock0_downblock1_downblock2_downblock3_resnet0_alignment'
    title = 'TADSR UNet Entry + DownBlock0 + DownBlock1 + DownBlock2 + DownBlock3 ResNet0 Bridge Alignment'
    if blocked:
        result = blocked_result(name, title, blocked)
        print('TADSR_UNET_ENTRY_DOWNBLOCK0_DOWNBLOCK1_DOWNBLOCK2_DOWNBLOCK3_RESNET0_ALIGNMENT:', result['status'])
        return 0 if result['status'] == 'PASS' else 1
    tester = bridge_tester()
    got = tester.run_entry_downblock0_downblock1_downblock2_downblock3_resnet0(
    ...
    result = {'status': status, 'diagnostics': diag, 'note': 'Local bridge: entry -> full down_blocks.0 -> full down_blocks.1 -> full down_blocks.2 -> down_blocks.3.resnets.0 only. Full down_blocks.3 and full UNet are not executed.'}
    write_report(name, title, result)
    print('TADSR_UNET_ENTRY_DOWNBLOCK0_DOWNBLOCK1_DOWNBLOCK2_DOWNBLOCK3_RESNET0_ALIGNMENT:', status)
