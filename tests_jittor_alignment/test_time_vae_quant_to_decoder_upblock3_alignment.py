...
def main() -> int:
    ...
    if blocked:
        result = blocked
        write_report('jittor_quant_to_decoder_upblock3_alignment', 'TimeAware VAE Quant To Decoder UpBlock3 Alignment', result)
        print(f"TIME_VAE_QUANT_TO_DECODER_UPBLOCK3_ALIGNMENT: {result['status']}")
        return 1

    tester = TimeVAEBlockTester(WEIGHTS)
    got = tester.run_quant_to_decoder_upblock3(oracle['real_moments'], return_intermediates=True)
    diagnostics = {
    ...
    result = {
        ...
        'target': 'moments -> posterior mode -> decoder up_blocks.3',
