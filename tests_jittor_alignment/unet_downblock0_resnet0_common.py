...
from jittor_tadsr_full.unet_2d_condition import TADSRUNetDownBlock0Resnet0Tester
from jittor_tadsr_full.utils import compare_arrays, write_json

OUT = Path('experiments/full_repro/unet_alignment')
ORACLE = OUT / 'oracle_tensors_unet_downblock0_resnet0'
META = ORACLE / 'unet_downblock0_resnet0_oracle_metadata.json'
...
def load_oracle():
    if not ORACLE.exists() or not META.exists() or not RESNET_WEIGHTS.exists():
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_downblock0_resnet0_oracle.py first.'}
...
def tester():
    return TADSRUNetDownBlock0Resnet0Tester(ENTRY_WEIGHTS, RESNET_WEIGHTS, metadata=load_json(META))
