...
from jittor_tadsr_full.unet_2d_condition import UNetResnetBlock2DTester, TADSRUNetDownBlock3Resnet1Tester, TADSRUNetDownBlock3Tester
from jittor_tadsr_full.utils import compare_arrays, write_json

OUT = Path('experiments/full_repro/unet_alignment')
ORACLE = OUT / 'oracle_tensors_unet_downblock3_resnet1'
META = ORACLE / 'unet_downblock3_resnet1_oracle_metadata.json'
...
def load_oracle():
    ...
    if missing:
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_downblock3_resnet1_oracle.py first.', 'missing': missing}
    tensors = {p.stem: np.load(p).astype(np.float32) for p in ORACLE.glob('*.npy')}
    if not tensors:
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_downblock3_resnet1_oracle.py first.'}
    return tensors, None
...
def metadata():
    return {
        ...
        'resnet1_metadata': load_json(META),
    }


def resnet1_tester():
    meta = load_json(META)
    return UNetResnetBlock2DTester(DOWN3_RESNET1_WEIGHTS, meta.get('resnet1_config', {}).get('prefix', 'down_blocks_3_resnets_1'), meta)


def bridge_tester():
    return TADSRUNetDownBlock3Resnet1Tester(
        ENTRY_WEIGHTS,
...
def downblock3_tester():
    return TADSRUNetDownBlock3Tester(
        ENTRY_WEIGHTS,
        DOWN0_RESNET0_WEIGHTS,
        DOWN0_ATTENTION0_WEIGHTS,
        DOWN0_RESNET1_WEIGHTS,
