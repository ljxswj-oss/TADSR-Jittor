...
def main() -> int:
    ...
    if blocked:
        result = blocked
        write_report('jittor_encoder_to_decoder_upblock2_alignment', 'TimeAware VAE Encoder To Decoder UpBlock2 Alignment', result)
        print(f"TIME_VAE_ENCODER_TO_DECODER_UPBLOCK2_ALIGNMENT: {result['status']}")
        return 1

    tester = TimeVAEBlockTester(WEIGHTS)
    got = tester.run_encoder_stage0123_mid_tail_quant_to_decoder_upblock2(
    ...
    result = {
        ...
        'target': 'encoder image path -> quant_conv -> decoder up_blocks.2',
