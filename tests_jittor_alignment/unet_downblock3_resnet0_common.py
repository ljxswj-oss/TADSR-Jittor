...
from jittor_tadsr_full.unet_2d_condition import UNetResnetBlock2DTester, TADSRUNetDownBlock3Resnet0Tester
from jittor_tadsr_full.utils import compare_arrays, write_json

OUT = Path('experiments/full_repro/unet_alignment')
ORACLE = OUT / 'oracle_tensors_unet_downblock3_resnet0'
META = ORACLE / 'unet_downblock3_resnet0_oracle_metadata.json'
...
def load_oracle():
    ...
    if missing:
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_downblock3_resnet0_oracle.py first.', 'missing': missing}
    tensors = {p.stem: np.load(p).astype(np.float32) for p in ORACLE.glob('*.npy')}
    if not tensors:
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_downblock3_resnet0_oracle.py first.'}
    return tensors, None
...
def resnet0_tester():
    meta = load_json(META)
    return UNetResnetBlock2DTester(DOWN3_RESNET0_WEIGHTS, meta.get('resnet0_config', {}).get('prefix', 'down_blocks_3_resnets_0'), meta)


def bridge_tester():
    return TADSRUNetDownBlock3Resnet0Tester(
        ENTRY_WEIGHTS,
        ...
        metadata={
            ...
            'resnet0_metadata': load_json(META),
        },
    )
