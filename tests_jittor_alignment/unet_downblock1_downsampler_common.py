...
from jittor_tadsr_full.unet_2d_condition import TADSRUNetDownBlock1DownsamplerTester, TADSRUNetDownBlock1Tester
from jittor_tadsr_full.utils import compare_arrays, write_json

OUT = Path('experiments/full_repro/unet_alignment')
ORACLE = OUT / 'oracle_tensors_unet_downblock1_downsampler'
META = ORACLE / 'unet_downblock1_downsampler_oracle_metadata.json'
...
def load_oracle():
    ...
    if missing:
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_downblock1_downsampler_oracle.py first.', 'missing': missing}
    tensors = {p.stem: np.load(p).astype(np.float32) for p in ORACLE.glob('*.npy')}
    if not tensors:
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_downblock1_downsampler_oracle.py first.'}
    return tensors, None
...
def downsampler_tester():
    return TADSRUNetDownBlock1DownsamplerTester(DOWN1_DOWNSAMPLER_WEIGHTS, metadata=load_json(META))


def downblock1_tester():
    return TADSRUNetDownBlock1Tester(
        ENTRY_WEIGHTS,
