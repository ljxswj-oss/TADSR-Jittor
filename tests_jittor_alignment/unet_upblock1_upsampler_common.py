...
from jittor_tadsr_full.unet_2d_condition import UNetUpsample2DTester, TADSRUNetUpBlock1UpsamplerTester
from unet_upblock1_attention2_common import (
    OUT,
    ENTRY_WEIGHTS,
...
from unet_upblock1_attention2_common import metadata as upblock1_attention2_metadata
from unet_upblock1_resnet0_common import squeeze_expected_if_needed

ORACLE = OUT / 'oracle_tensors_unet_upblock1_upsampler'
META = ORACLE / 'unet_upblock1_upsampler_oracle_metadata.json'
UP1_UPSAMPLER_WEIGHTS = OUT / 'converted_unet_upblock1_upsampler_effective_weights.npz'
...
def load_oracle():
    ...
    if missing:
        return None, {
            'status': 'BLOCKED',
            'reason': 'Run python3 tools/export_tadsr_unet_upblock1_upsampler_oracle.py first.',
            'missing': missing,
        }
    tensors = {p.stem: np.load(p).astype(np.float32) for p in ORACLE.glob('*.npy')}
    if not tensors:
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_upblock1_upsampler_oracle.py first.'}
    return tensors, None


def metadata():
    meta = upblock1_attention2_metadata()
    meta['upblock1_upsampler_metadata'] = load_json(META)
    return meta


def upsampler_tester():
    meta = load_json(META)
    prefix = meta.get('upsampler_config', {}).get('prefix', 'up_blocks_1_upsamplers_0')
    return UNetUpsample2DTester(UP1_UPSAMPLER_WEIGHTS, prefix, meta)


def bridge_tester():
    return TADSRUNetUpBlock1UpsamplerTester(
        ENTRY_WEIGHTS,
        DOWN0_RESNET0_WEIGHTS,
        DOWN0_ATTENTION0_WEIGHTS,
        DOWN0_RESNET1_WEIGHTS,
