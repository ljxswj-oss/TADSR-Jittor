...
from jittor_tadsr_full.unet_2d_condition import UNetAttention0Transformer2DTester, TADSRUNetDownBlock1Attention1Tester
from jittor_tadsr_full.utils import compare_arrays, write_json

OUT = Path('experiments/full_repro/unet_alignment')
ORACLE = OUT / 'oracle_tensors_unet_downblock1_attention1'
META = ORACLE / 'unet_downblock1_attention1_oracle_metadata.json'
...
def load_oracle():
    required = [ORACLE, META, ENTRY_WEIGHTS, DOWN0_RESNET0_WEIGHTS, DOWN0_ATTENTION0_WEIGHTS, DOWN0_RESNET1_WEIGHTS, DOWN0_ATTENTION1_WEIGHTS, DOWN0_DOWNSAMPLER_WEIGHTS, DOWN1_RESNET0_WEIGHTS, DOWN1_ATTENTION0_WEIGHTS, DOWN1_RESNET1_WEIGHTS, DOWN1_ATTENTION1_WEIGHTS]
    missing = [str(p) for p in required if not p.exists()]
    if missing:
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_downblock1_attention1_oracle.py first.', 'missing': missing}
    tensors = {p.stem: np.load(p).astype(np.float32) for p in ORACLE.glob('*.npy')}
    if not tensors:
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_downblock1_attention1_oracle.py first.'}
    return tensors, None
...
def attention1_tester():
    return UNetAttention0Transformer2DTester(DOWN1_ATTENTION1_WEIGHTS, metadata=load_json(META))


def bridge_tester():
    return TADSRUNetDownBlock1Attention1Tester(
        ENTRY_WEIGHTS,
