...
from jittor_tadsr_full.unet_2d_condition import UNetAttention0Transformer2DTester, TADSRUNetUpBlock2Attention0Tester
...
from unet_upblock2_resnet0_common import metadata as upblock2_resnet0_metadata

ORACLE = OUT / 'oracle_tensors_unet_upblock2_attention0'
META = ORACLE / 'unet_upblock2_attention0_oracle_metadata.json'
UP2_ATTENTION0_WEIGHTS = OUT / 'converted_unet_upblock2_attention0_effective_weights.npz'
...
def load_oracle():
    ...
    if missing:
        return None, {
            'status': 'BLOCKED',
            'reason': 'Run python3 tools/export_tadsr_unet_upblock2_attention0_oracle.py first.',
            'missing': missing,
        }
    tensors = {p.stem: np.load(p).astype(np.float32) for p in ORACLE.glob('*.npy')}
    if not tensors:
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_upblock2_attention0_oracle.py first.'}
    return tensors, None


def metadata():
    meta = upblock2_resnet0_metadata()
    meta['upblock2_attention0_metadata'] = load_json(META)
    return meta


def attention0_tester():
    return UNetAttention0Transformer2DTester(UP2_ATTENTION0_WEIGHTS, metadata=load_json(META))


def bridge_tester():
    return TADSRUNetUpBlock2Attention0Tester(
        ENTRY_WEIGHTS,
        DOWN0_RESNET0_WEIGHTS,
        DOWN0_ATTENTION0_WEIGHTS,
        DOWN0_RESNET1_WEIGHTS,
