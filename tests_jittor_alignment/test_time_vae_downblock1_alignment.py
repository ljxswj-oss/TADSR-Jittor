...
def main() -> int:
    ...
    if not inp_path.exists() or not out_path.exists():
        ...
        after_resnet0 = tester.run_resnet0(inputs['downblock1_input'], inputs['downblock1_temb'])
        after_resnet1 = tester.run_resnet1(after_resnet0, inputs['downblock1_temb'])
        ...
        diagnostics = {
            ...
            'final_downblock1': add_metrics(final, outputs['downblock1_output'], 1e-4),
            'direct_pytorch_downblock1_reference': add_metrics(outputs['downblock1_direct_output'], outputs['downblock1_output'], 1e-7) if 'downblock1_direct_output' in outputs.files else None,
        }
        diagnostics = {k: v for k, v in diagnostics.items() if v is not None}
        final_metrics = diagnostics['final_downblock1']
        result = {
            'status': status_from_metrics(final_metrics),
            'target': 'encoder.down_blocks.1',
            'module_order': ['resnets.0', 'resnets.1', 'downsamplers.0'],
            'metrics': final_metrics,
            'diagnostics': diagnostics,
            'weights': str(WEIGHTS),
        }
    write_json(OUT / 'jittor_downblock1_alignment.json', result)
    md = ['# TimeAware VAE Full DownBlock1 Alignment', '', f"Status: **{result['status']}**"]
    ...
    (OUT / 'jittor_downblock1_alignment.md').write_text('\n'.join(md) + '\n', encoding='utf-8')
    print(f"TIME_VAE_DOWNBLOCK1_ALIGNMENT: {result['status']}")
    if result.get('metrics'):
