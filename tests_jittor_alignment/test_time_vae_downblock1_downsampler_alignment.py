...
def main() -> int:
    ...
    if not inp_path.exists() or not out_path.exists():
        ...
        if 'downsampler0_input' not in inputs.files or 'downsampler0_output' not in outputs.files:
            result = {'status': 'NOT_APPLICABLE', 'reason': 'official encoder.down_blocks.1 has no downsampler oracle'}
        else:
            ...
            result = {
                ...
                'target': 'encoder.down_blocks.1.downsamplers.0',
                'padding_note': 'Diffusers Downsample2D uses right/bottom asymmetric zero padding before stride-2 conv when padding=0.',
                'metrics': metrics,
                'weights': str(WEIGHTS),
            }
    write_report('jittor_downblock1_downsampler_alignment', 'TimeAware VAE DownBlock1 Downsampler Alignment', result)
    print(f"TIME_VAE_DOWNBLOCK1_DOWNSAMPLER_ALIGNMENT: {result['status']}")
    if result.get('metrics'):
