...
from jittor_tadsr_full.unet_2d_condition import UNetResnetBlock2DTester, TADSRUNetMidBlockResnet0Tester
from jittor_tadsr_full.utils import compare_arrays, write_json

OUT = Path('experiments/full_repro/unet_alignment')
ORACLE = OUT / 'oracle_tensors_unet_midblock_resnet0'
META = ORACLE / 'unet_midblock_resnet0_oracle_metadata.json'
...
DOWN1_META = OUT / 'oracle_tensors_unet_downblock1_downsampler' / 'unet_downblock1_downsampler_oracle_metadata.json'
DOWN2_RESNET0_META = OUT / 'oracle_tensors_unet_downblock2_resnet0' / 'unet_downblock2_resnet0_oracle_metadata.json'
DOWN2_ATTENTION0_META = OUT / 'oracle_tensors_unet_downblock2_attention0' / 'unet_downblock2_attention0_oracle_metadata.json'
DOWN2_RESNET1_META = OUT / 'oracle_tensors_unet_downblock2_resnet1' / 'unet_downblock2_resnet1_oracle_metadata.json'
DOWN2_ATTENTION1_META = OUT / 'oracle_tensors_unet_downblock2_attention1' / 'unet_downblock2_attention1_oracle_metadata.json'
DOWN2_META = OUT / 'oracle_tensors_unet_downblock2_downsampler' / 'unet_downblock2_downsampler_oracle_metadata.json'
DOWN3_RESNET0_META = OUT / 'oracle_tensors_unet_downblock3_resnet0' / 'unet_downblock3_resnet0_oracle_metadata.json'
DOWN3_RESNET1_META = OUT / 'oracle_tensors_unet_downblock3_resnet1' / 'unet_downblock3_resnet1_oracle_metadata.json'
...
DOWN2_RESNET1_WEIGHTS = OUT / 'converted_unet_downblock2_resnet1_effective_weights.npz'
DOWN2_ATTENTION1_WEIGHTS = OUT / 'converted_unet_downblock2_attention1_effective_weights.npz'
DOWN2_DOWNSAMPLER_WEIGHTS = OUT / 'converted_unet_downblock2_downsampler_effective_weights.npz'
DOWN3_RESNET0_WEIGHTS = OUT / 'converted_unet_downblock3_resnet0_effective_weights.npz'
DOWN3_RESNET1_WEIGHTS = OUT / 'converted_unet_downblock3_resnet1_effective_weights.npz'
MID_RESNET0_WEIGHTS = OUT / 'converted_unet_midblock_resnet0_effective_weights.npz'
...
def load_oracle():
    ...
    if missing:
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_midblock_resnet0_oracle.py first.', 'missing': missing}
    tensors = {p.stem: np.load(p).astype(np.float32) for p in ORACLE.glob('*.npy')}
    if not tensors:
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_midblock_resnet0_oracle.py first.'}
    return tensors, None
        ...
        'downblock3_metadata': {
            'downblock2_metadata': {
                'downblock1_metadata': {
                    ...
                    'downsampler_metadata': load_json(DOWN1_META),
                },
                'resnet0_metadata': load_json(DOWN2_RESNET0_META),
                'attention0_metadata': load_json(DOWN2_ATTENTION0_META),
                'resnet1_metadata': load_json(DOWN2_RESNET1_META),
                'attention1_metadata': load_json(DOWN2_ATTENTION1_META),
                'downsampler_metadata': load_json(DOWN2_META),
            },
            'resnet0_metadata': load_json(DOWN3_RESNET0_META),
            'resnet1_metadata': load_json(DOWN3_RESNET1_META),
        },
        'midblock_resnet0_metadata': load_json(META),
    }


def resnet0_tester():
    meta = load_json(META)
    return UNetResnetBlock2DTester(MID_RESNET0_WEIGHTS, meta.get('resnet0_config', {}).get('prefix', 'mid_block_resnets_0'), meta)


def bridge_tester():
    return TADSRUNetMidBlockResnet0Tester(
        ENTRY_WEIGHTS,
...
def squeeze_expected_if_needed(actual, expected):
    if getattr(actual, 'ndim', 0) == 2 and getattr(expected, 'ndim', 0) == 4 and expected.shape[2:] == (1, 1):
        return expected[:, :, 0, 0]
    return expected
