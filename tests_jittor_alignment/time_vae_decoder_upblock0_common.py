...
ORACLE = OUT / 'oracle_tensors_decoder_upblock0'
...
def metadata():
    return load_json(ORACLE / 'decoder_upblock0_oracle_metadata.json')
...
def load_oracle():
    if not ORACLE.exists():
        return None, {'status': 'BLOCKED', 'reason': 'Decoder upblock0 oracle tensors missing'}
...
def failure_analysis():
    return [
        'Check audit_time_vae_decoder_upblocks.json for upblock0 module order and upsampler behavior.',
        'Check decoder.up_blocks.0 converted NPZ keys and prefix resolution.',
