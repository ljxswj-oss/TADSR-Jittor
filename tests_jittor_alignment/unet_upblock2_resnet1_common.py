...
from jittor_tadsr_full.unet_2d_condition import UNetResnetBlock2DTester, TADSRUNetUpBlock2Resnet1Tester
from unet_upblock2_attention0_common import (
    OUT,
    ENTRY_WEIGHTS,
...
from unet_upblock2_attention0_common import metadata as upblock2_attention0_metadata
from unet_upblock1_resnet0_common import squeeze_expected_if_needed

ORACLE = OUT / 'oracle_tensors_unet_upblock2_resnet1'
META = ORACLE / 'unet_upblock2_resnet1_oracle_metadata.json'
UP2_RESNET1_WEIGHTS = OUT / 'converted_unet_upblock2_resnet1_effective_weights.npz'
...
def load_oracle():
    ...
    if missing:
        return None, {
            'status': 'BLOCKED',
            'reason': 'Run python3 tools/export_tadsr_unet_upblock2_resnet1_oracle.py first.',
            'missing': missing,
        }
    tensors = {p.stem: np.load(p).astype(np.float32) for p in ORACLE.glob('*.npy')}
    if not tensors:
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_upblock2_resnet1_oracle.py first.'}
    return tensors, None


def metadata():
    meta = upblock2_attention0_metadata()
    meta['upblock2_resnet1_metadata'] = load_json(META)
    return meta


def resnet1_tester():
    meta = load_json(META)
    prefix = meta.get('resnet1_config', {}).get('prefix', 'up_blocks_2_resnets_1')
    return UNetResnetBlock2DTester(UP2_RESNET1_WEIGHTS, prefix, meta)


def bridge_tester():
    return TADSRUNetUpBlock2Resnet1Tester(
        ENTRY_WEIGHTS,
        DOWN0_RESNET0_WEIGHTS,
        DOWN0_ATTENTION0_WEIGHTS,
        DOWN0_RESNET1_WEIGHTS,
