...
def main() -> int:
    ...
    if not inp_path.exists() or not out_path.exists():
        ...
        got = tester.run_resnet0(inputs['resnet0_input'], inputs['downblock1_temb'])
        expected = outputs['resnet0_output']
        metrics = add_metrics(got, expected, 1e-4)
        result = {
            'status': status_from_metrics(metrics),
            'target': 'encoder.down_blocks.1.resnets.0',
            'channel_change': '128->256',
            'uses_conv_shortcut': True,
            'metrics': metrics,
            'weights': str(WEIGHTS),
        }
    write_report('jittor_downblock1_resnet0_alignment', 'TimeAware VAE DownBlock1 ResNet0 Alignment', result)
    print(f"TIME_VAE_DOWNBLOCK1_RESNET0_ALIGNMENT: {result['status']}")
    if result.get('metrics'):
