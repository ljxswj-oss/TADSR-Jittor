...
def main() -> int:
    ...
    name = 'jittor_unet_entry_downblocks_midblock_upblock0_upblock1_upblock2_resnet0_alignment'
    title = 'TADSR UNet Entry -> DownBlocks -> MidBlock -> UpBlock0 -> UpBlock1 -> UpBlock2 Resnet0 Bridge Alignment'
    if blocked:
        result = blocked_result(name, title, blocked)
        print('TADSR_UNET_ENTRY_DOWNBLOCKS_MIDBLOCK_UPBLOCK0_UPBLOCK1_UPBLOCK2_RESNET0_ALIGNMENT:', result['status'])
        return 0 if result['status'] == 'PASS' else 1

    tester = bridge_tester()
    got = tester.run_entry_downblocks_midblock_upblock0_upblock1_upblock2_resnet0(
    ...
    expected = {
        ...
        'upblock2_resnet0_res_hidden': 'entry_upblock2_resnet0_res_hidden',
        'upblock2_resnet0_concat_input': 'entry_upblock2_resnet0_concat_input',
        'upblock2_resnet0_output': 'entry_upblock2_resnet0_output',
    }

    diag = {}
    for stage, tensor_name in expected.items():
        if stage in {'upblock2_resnet0_res_hidden', 'upblock2_resnet0_concat_input'}:
            tol = 1e-4
        elif stage in {'conv_in', 'time_embedding'}:
            tol = 1e-4
        else:
            tol = 2e-3
    ...
    result = {
        ...
        'note': (
            ...
            'It deliberately stops before up_blocks.2.attentions.0 and does not mark full up_blocks.2 complete.'
        ),
    }
    write_report(name, title, result)
    print('TADSR_UNET_ENTRY_DOWNBLOCKS_MIDBLOCK_UPBLOCK0_UPBLOCK1_UPBLOCK2_RESNET0_ALIGNMENT:', status)
