...
from unet_downblock1_resnet1_common import add_metrics, blocked_result, bridge_tester, load_oracle, status_from_diagnostics, write_report


def main() -> int:
    tensors, blocked = load_oracle()
    name = 'jittor_unet_entry_downblock0_downblock1_resnet0_attention0_resnet1_alignment'
    title = 'TADSR UNet Entry + DownBlock0 + DownBlock1 ResNet0 + Attention0 + ResNet1 Bridge Alignment'
    if blocked:
        result = blocked_result(name, title, blocked)
        print('TADSR_UNET_ENTRY_DOWNBLOCK0_DOWNBLOCK1_RESNET0_ATTENTION0_RESNET1_ALIGNMENT:', result['status'])
        return 0 if result['status'] == 'PASS' else 1
    tester = bridge_tester()
    got = tester.run_entry_downblock0_downblock1_resnet0_attention0_resnet1(
    ...
    write_report(name, title, {'status': status, 'diagnostics': diag, 'note': 'Local bridge stops after down_blocks.1.resnets.1. Full down_blocks.1 and full UNet are not executed.'})
    print('TADSR_UNET_ENTRY_DOWNBLOCK0_DOWNBLOCK1_RESNET0_ATTENTION0_RESNET1_ALIGNMENT:', status)
