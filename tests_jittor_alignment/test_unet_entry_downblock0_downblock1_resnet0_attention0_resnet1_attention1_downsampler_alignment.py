...
def main() -> int:
    ...
    name = 'jittor_unet_entry_downblock0_downblock1_resnet0_attention0_resnet1_attention1_downsampler_alignment'
    title = 'TADSR UNet Entry + DownBlock0 + DownBlock1 ResNet0 + Attention0 + ResNet1 + Attention1 + Downsampler Bridge Alignment'
    if blocked:
        result = blocked_result(name, title, blocked)
        print('TADSR_UNET_ENTRY_DOWNBLOCK0_DOWNBLOCK1_RESNET0_ATTENTION0_RESNET1_ATTENTION1_DOWNSAMPLER_ALIGNMENT:', result['status'])
        return 0 if result['status'] == 'PASS' else 1
    tester = downblock1_tester()
    got = tester.run_entry_downblock0_downblock1_resnet0_attention0_resnet1_attention1_downsampler(
    ...
    write_report(name, title, {'status': status, 'diagnostics': diag, 'note': 'Local bridge stops after down_blocks.1.downsamplers.0. Full UNet is not executed.'})
    print('TADSR_UNET_ENTRY_DOWNBLOCK0_DOWNBLOCK1_RESNET0_ATTENTION0_RESNET1_ATTENTION1_DOWNSAMPLER_ALIGNMENT:', status)
