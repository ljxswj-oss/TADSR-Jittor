...
def main() -> int:
    ...
    if not inp_path.exists() or not out_path.exists():
        ...
        got = tester.run_encoder_stage01(inputs['stage01_input'], inputs['downblock1_temb'])
        expected = outputs['stage01_output']
        metrics = add_metrics(got, expected, 1e-4)
        result = {
            'status': status_from_metrics(metrics),
            'target': 'encoder.conv_in + encoder.down_blocks.0 + encoder.down_blocks.1',
            'module_order': ['encoder.conv_in', 'encoder.down_blocks.0', 'encoder.down_blocks.1'],
            'metrics': metrics,
            'weights': str(WEIGHTS),
        }
    write_json(OUT / 'jittor_encoder_stage01_alignment.json', result)
    md = ['# TimeAware VAE Encoder Stage01 Alignment', '', f"Status: **{result['status']}**"]
    ...
    (OUT / 'jittor_encoder_stage01_alignment.md').write_text('\n'.join(md) + '\n', encoding='utf-8')
    print(f"TIME_VAE_ENCODER_STAGE01_ALIGNMENT: {result['status']}")
    if result.get('metrics'):
