...
ORACLE = OUT / 'oracle_tensors_decoder_entry'
...
def load_oracle():
    if not ORACLE.exists():
        return None, {'status': 'BLOCKED', 'reason': 'Decoder entry oracle tensors missing'}
...
def failure_analysis():
    return [
        'Check decoder_entry_oracle_metadata.json for post_quant_conv and decoder.conv_in shape/padding.',
        'Check converted NPZ keys post_quant_conv__* and decoder__conv_in__*.',
        'Check posterior split axis, logvar clamp and deterministic mode(mean) path.',
