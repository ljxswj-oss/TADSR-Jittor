...
from jittor_tadsr_full.unet_2d_condition import UNetResnetBlock2DTester, TADSRUNetUpBlock1Resnet2Tester
...
from unet_upblock1_attention1_common import metadata as upblock1_attention1_metadata
from unet_upblock1_resnet0_common import squeeze_expected_if_needed

ORACLE = OUT / 'oracle_tensors_unet_upblock1_resnet2'
META = ORACLE / 'unet_upblock1_resnet2_oracle_metadata.json'
UP1_RESNET2_WEIGHTS = OUT / 'converted_unet_upblock1_resnet2_effective_weights.npz'
...
def load_oracle():
    ...
    if missing:
        return None, {
            'status': 'BLOCKED',
            'reason': 'Run python3 tools/export_tadsr_unet_upblock1_resnet2_oracle.py first.',
            'missing': missing,
        }
    tensors = {p.stem: np.load(p).astype(np.float32) for p in ORACLE.glob('*.npy')}
    if not tensors:
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_upblock1_resnet2_oracle.py first.'}
    return tensors, None


def metadata():
    meta = upblock1_attention1_metadata()
    meta['upblock1_resnet2_metadata'] = load_json(META)
    return meta


def resnet2_tester():
    meta = load_json(META)
    prefix = meta.get('resnet2_config', {}).get('prefix', 'up_blocks_1_resnets_2')
    return UNetResnetBlock2DTester(UP1_RESNET2_WEIGHTS, prefix, meta)


def bridge_tester():
    return TADSRUNetUpBlock1Resnet2Tester(
        ENTRY_WEIGHTS,
        DOWN0_RESNET0_WEIGHTS,
        DOWN0_ATTENTION0_WEIGHTS,
        DOWN0_RESNET1_WEIGHTS,
