...
from jittor_tadsr_full.unet_2d_condition import UNetUpsample2DTester, TADSRUNetUpBlock2UpsamplerTester
...
from unet_upblock2_attention2_common import metadata as upblock2_attention2_metadata
from unet_upblock1_resnet0_common import squeeze_expected_if_needed

ORACLE = OUT / 'oracle_tensors_unet_upblock2_upsampler'
META = ORACLE / 'unet_upblock2_upsampler_oracle_metadata.json'
UP2_UPSAMPLER_WEIGHTS = OUT / 'converted_unet_upblock2_upsampler_effective_weights.npz'
...
def load_oracle():
    ...
    if missing:
        return None, {
            'status': 'BLOCKED',
            'reason': 'Run python3 tools/export_tadsr_unet_upblock2_upsampler_oracle.py first.',
            'missing': missing,
        }
    tensors = {p.stem: np.load(p).astype(np.float32) for p in ORACLE.glob('*.npy')}
    if not tensors:
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_upblock2_upsampler_oracle.py first.'}
    return tensors, None


def metadata():
    meta = upblock2_attention2_metadata()
    meta['upblock2_upsampler_metadata'] = load_json(META)
    return meta


def upsampler_tester():
    meta = load_json(META)
    prefix = meta.get('upsampler_config', {}).get('prefix', 'up_blocks_2_upsamplers_0')
    return UNetUpsample2DTester(UP2_UPSAMPLER_WEIGHTS, prefix, meta)


def bridge_tester():
    return TADSRUNetUpBlock2UpsamplerTester(
        ENTRY_WEIGHTS,
        DOWN0_RESNET0_WEIGHTS,
        DOWN0_ATTENTION0_WEIGHTS,
        DOWN0_RESNET1_WEIGHTS,
