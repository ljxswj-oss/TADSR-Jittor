...
from jittor_tadsr_full.unet_2d_condition import UNetResnetBlock2DTester, TADSRUNetUpBlock3Resnet0Tester
...
from unet_upblock2_local_common import metadata as upblock2_local_metadata
from unet_upblock1_resnet0_common import squeeze_expected_if_needed

ORACLE = OUT / 'oracle_tensors_unet_upblock3_resnet0'
META = ORACLE / 'unet_upblock3_resnet0_oracle_metadata.json'
UP3_RESNET0_WEIGHTS = OUT / 'converted_unet_upblock3_resnet0_effective_weights.npz'
...
def load_oracle():
    ...
    if missing:
        return None, {
            'status': 'BLOCKED',
            'reason': 'Run python3 tools/export_tadsr_unet_upblock3_resnet0_oracle.py first.',
            'missing': missing,
        }
    tensors = {p.stem: np.load(p).astype(np.float32) for p in ORACLE.glob('*.npy')}
    if not tensors:
        return None, {'status': 'BLOCKED', 'reason': 'Run python3 tools/export_tadsr_unet_upblock3_resnet0_oracle.py first.'}
    return tensors, None


def metadata():
    meta = upblock2_local_metadata()
    meta['upblock3_resnet0_metadata'] = load_json(META)
    return meta


def resnet0_tester():
    meta = load_json(META)
    prefix = meta.get('resnet0_config', {}).get('prefix', 'up_blocks_3_resnets_0')
    return UNetResnetBlock2DTester(UP3_RESNET0_WEIGHTS, prefix, meta)


def bridge_tester():
    return TADSRUNetUpBlock3Resnet0Tester(
        ENTRY_WEIGHTS,
        DOWN0_RESNET0_WEIGHTS,
        DOWN0_ATTENTION0_WEIGHTS,
        DOWN0_RESNET1_WEIGHTS,
