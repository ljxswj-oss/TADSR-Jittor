...
from unet_upblock1_upsampler_common import add_metrics, blocked_result, bridge_tester, load_oracle, status_from_diagnostics, write_report


def main() -> int:
    tensors, blocked = load_oracle()
    name = 'jittor_unet_entry_downblocks_midblock_upblock0_upblock1_through_upsampler_alignment'
    title = 'TADSR UNet Entry -> DownBlocks -> MidBlock -> UpBlock0 -> UpBlock1 Through Upsampler Bridge Alignment'
    if blocked:
        result = blocked_result(name, title, blocked)
        print('TADSR_UNET_ENTRY_DOWNBLOCKS_MIDBLOCK_UPBLOCK0_UPBLOCK1_THROUGH_UPSAMPLER_ALIGNMENT:', result['status'])
        return 0 if result['status'] == 'PASS' else 1
    tester = bridge_tester()
    got = tester.run_entry_downblocks_midblock_upblock0_upblock1_through_upsampler(
    ...
    expected = {
        ...
        'upblock1_attention2_output': 'entry_upblock1_attention2_output',
        'upblock1_upsampler_input': 'entry_upblock1_upsampler_input',
        'upblock1_upsampler_interpolation_output': 'entry_upblock1_upsampler_interpolation_output',
        'upblock1_upsampler_conv_output': 'entry_upblock1_upsampler_conv_output',
        'upblock1_upsampler_output': 'entry_upblock1_upsampler_output',
    }
    diag = {}
    for stage, path in expected.items():
        tol = 1e-4 if stage in {'conv_in', 'time_embedding'} else 2e-3
        if stage == 'upblock1_upsampler_interpolation_output':
            tol = 2e-3
    ...
    result = {
        ...
        'note': (
            ...
            '-> up_blocks.1.resnets/attentions through upsamplers.0. It deliberately stops before up_blocks.2 '
            'and does not mark full up_blocks.1 aggregate complete.'
        ),
    }
    write_report(name, title, result)
    print('TADSR_UNET_ENTRY_DOWNBLOCKS_MIDBLOCK_UPBLOCK0_UPBLOCK1_THROUGH_UPSAMPLER_ALIGNMENT:', status)
