...
ORACLE = OUT / 'oracle_tensors_decoder_upblock2'
...
def metadata():
    return load_json(ORACLE / 'decoder_upblock2_oracle_metadata.json')
...
def load_oracle():
    if not ORACLE.exists():
        return None, {'status': 'BLOCKED', 'reason': 'Decoder upblock2 oracle tensors missing'}
    tensor_files = sorted(ORACLE.glob('*.npy'))
    if not tensor_files:
        return None, {'status': 'BLOCKED', 'reason': 'Decoder upblock2 .npy oracle tensors missing; run tools/export_time_vae_decoder_upblock2_oracle.py before alignment tests'}
...
def failure_analysis():
    return [
        'Check audit_time_vae_decoder_upblock2.json for module order, channel change, and upsampler behavior.',
        'Check decoder.up_blocks.2.resnets.0 converted conv_shortcut weights; this block changes channels from 512 to 256.',
        'Check decoder.up_blocks.2 converted NPZ keys and prefix resolution.',
        ...
        'Check that decoder.up_blocks.1 output is used as decoder.up_blocks.2 input in bridge tests.',
