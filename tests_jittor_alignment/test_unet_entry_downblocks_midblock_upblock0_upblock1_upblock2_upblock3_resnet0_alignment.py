...
def main() -> int:
    ...
    name = 'jittor_unet_entry_downblocks_midblock_upblock0_upblock1_upblock2_upblock3_resnet0_alignment'
    title = 'TADSR UNet Entry -> DownBlocks -> MidBlock -> UpBlock0 -> UpBlock1 -> UpBlock2 -> UpBlock3 Resnet0 Bridge Alignment'
    if blocked:
        result = blocked_result(name, title, blocked)
        print('TADSR_UNET_ENTRY_DOWNBLOCKS_MIDBLOCK_UPBLOCK0_UPBLOCK1_UPBLOCK2_UPBLOCK3_RESNET0_ALIGNMENT:', result['status'])
        return 0 if result['status'] == 'PASS' else 1

    tester = bridge_tester()
    got = tester.run_entry_downblocks_midblock_upblock0_upblock1_upblock2_upblock3_resnet0(
    ...
    expected = {
        ...
        'upblock3_resnet0_res_hidden': 'entry_upblock3_resnet0_res_hidden',
        'upblock3_resnet0_concat_input': 'entry_upblock3_resnet0_concat_input',
    ...
    for stage, tensor_name in expected.items():
        ...
        if stage == 'upblock3_resnet0_concat_input':
            tol = 2e-3
    ...
    result = {
        ...
        'note': (
            ...
            'up_blocks.2 -> up_blocks.3.resnets.0. It deliberately stops before '
            'up_blocks.3.attentions.0/resnets.1, full up_blocks.3, full UNet '
            'forward, runtime LoRA, and full TADSR inference.'
        ),
    }
    write_report(name, title, result)
    print('TADSR_UNET_ENTRY_DOWNBLOCKS_MIDBLOCK_UPBLOCK0_UPBLOCK1_UPBLOCK2_UPBLOCK3_RESNET0_ALIGNMENT:', status)
