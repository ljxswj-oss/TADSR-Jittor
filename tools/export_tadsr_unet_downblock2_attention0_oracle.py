...
ORACLE_DIR = OUT_DIR / 'oracle_tensors_unet_downblock2_attention0'
META_JSON = ORACLE_DIR / 'unet_downblock2_attention0_oracle_metadata.json'
SUMMARY_TXT = ORACLE_DIR / 'oracle_summary.txt'
EFFECTIVE_WEIGHTS = OUT_DIR / 'converted_unet_downblock2_attention0_effective_weights.npz'
PREV_RESNET0_ORACLE = OUT_DIR / 'oracle_tensors_unet_downblock2_resnet0' / 'entry_downblock2_resnet0_output.npy'
...
def main() -> int:
    ...
    save_tensor(saved, 'synthetic_downblock2_attention0_input', synthetic_hidden)
    save_tensor(saved, 'synthetic_downblock2_attention0_encoder_hidden_states', encoder_hidden_states)
    for k, v in synthetic_manual.items():
        save_tensor(saved, f'synthetic_downblock2_attention0_{k}', v)
    save_tensor(saved, 'synthetic_downblock2_attention0_official_output', synthetic_official)
    ...
    for k, v in entry_manual.items():
        save_tensor(saved, f'entry_downblock2_attention0_{k}', v)
    save_tensor(saved, 'entry_downblock2_attention0_official_output', entry_official)
    arrays = {}
    emeta = {}
    prefix = 'down_blocks_2_attentions_0'
    ...
    previous_resnet0_compare = {'previous_downblock2_resnet0_oracle_tensor_available': False}
    if PREV_RESNET0_ORACLE.exists():
        ...
        previous_resnet0_compare = {
            'previous_downblock2_resnet0_oracle_tensor_available': True,
    ...
    metadata = {
        ...
        'path_a_isolated_attention0': {'status': 'PASS', 'input_shape': h_shape, 'encoder_shape': encoder_shape, 'output_shape': list(synthetic_official.shape)},
        'path_b_entry_downblock0_downblock1_resnet0_attention0': {'status': 'PASS', 'sample_shape': sample_shape, 'resnet0_shape': list(resnet0_out.shape), 'output_shape': list(entry_official.shape)},
        'previous_downblock2_resnet0_compare': previous_resnet0_compare,
        'partial_downblock2_residual_preview': {
            'resnets0_output_shape': list(resnet0_out.shape),
            'attentions0_output_shape': list(entry_official.shape),
        ...
        'saved_tensors': saved,
        'markers': {
            'TADSR_UNET_DOWNBLOCK2_ATTENTION0_ORACLE_TENSORS': 'PASS',
            'TADSR_UNET_DOWNBLOCK2_ATTENTION0_EFFECTIVE_WEIGHTS': 'PASS',
        },
        'not_in_scope': ['full down_blocks.2', 'down_blocks.2.resnets.1', 'down_blocks.2.attentions.1', 'down_blocks.2.downsamplers.0', 'full UNet forward', 'full TADSR inference'],
    }
    META_JSON.write_text(json.dumps(metadata, indent=2, default=str), encoding='utf-8')
    lines = [
        '# TADSR UNet down_blocks.2.attentions.0 Oracle Export',
        '',
        'TADSR_UNET_DOWNBLOCK2_ATTENTION0_ORACLE_TENSORS: PASS',
        'TADSR_UNET_DOWNBLOCK2_ATTENTION0_EFFECTIVE_WEIGHTS: PASS',
        '',
        ...
        f"Path A synthetic output shape: `{metadata['path_a_isolated_attention0']['output_shape']}`",
        f"Path B entry->downblock0->downblock1->downblock2.resnet0->attention0 output shape: `{metadata['path_b_entry_downblock0_downblock1_resnet0_attention0']['output_shape']}`",
        ...
        '',
        'This export stops before down_blocks.2.resnets.1, down_blocks.2.attentions.1, down_blocks.2.downsamplers.0 and full UNet.',
    ]
    SUMMARY_TXT.write_text('\n'.join(lines) + '\n', encoding='utf-8')
    print('TADSR_UNET_DOWNBLOCK2_ATTENTION0_ORACLE_TENSORS: PASS')
    print('TADSR_UNET_DOWNBLOCK2_ATTENTION0_EFFECTIVE_WEIGHTS: PASS')
