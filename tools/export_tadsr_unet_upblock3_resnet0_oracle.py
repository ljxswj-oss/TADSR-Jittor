...
ORACLE_DIR = OUT_DIR / 'oracle_tensors_unet_upblock3_resnet0'
META_JSON = ORACLE_DIR / 'unet_upblock3_resnet0_oracle_metadata.json'
SUMMARY_TXT = ORACLE_DIR / 'oracle_summary.txt'
EFFECTIVE_WEIGHTS = OUT_DIR / 'converted_unet_upblock3_resnet0_effective_weights.npz'
PREV_UPBLOCK2_LOCAL_ORACLE = OUT_DIR / 'oracle_tensors_unet_upblock2_local' / 'local_upblock2_hidden_states_output.npy'
...
def main() -> int:
    ...
    with torch.no_grad():
        ...
        entry_inter = resnet_manual(resnet0, concat0, ctx['time_embedding'])
        entry_official = resnet0(concat0, ctx['time_embedding'])

        h_shape = list(up2_output.shape)
    ...
    saved = {}
    for name, tensor in [
        ('synthetic_upblock3_resnet0_hidden_input', synthetic_hidden),
        ('synthetic_upblock3_resnet0_res_hidden', synthetic_res),
        ('synthetic_upblock3_resnet0_concat_input', synthetic_concat),
        ('synthetic_upblock3_resnet0_temb', synthetic_temb),
    ]:
        save_tensor(saved, name, tensor)
    save_resnet_intermediates(saved, 'synthetic_upblock3_resnet0', synthetic_inter, synthetic_official)
    ...
    for name, tensor in [
        ...
        save_tensor(saved, name, tensor)
    save_resnet_intermediates(saved, 'entry_upblock3_resnet0', entry_inter, entry_official)

    effective = {}
    effective_meta = {}
    prefix = 'up_blocks_3_resnets_0'
    ...
    previous_upblock2_compare = {'previous_upblock2_local_oracle_tensor_available': False}
    if PREV_UPBLOCK2_LOCAL_ORACLE.exists():
        prev = np.load(PREV_UPBLOCK2_LOCAL_ORACLE)
        curr = to_np(up2_output)
        previous_upblock2_compare = {
            'previous_upblock2_local_oracle_tensor_available': True,
    ...
    metadata = {
        ...
        'upblock3_resnet0_residual_input_shape': list(res0_hidden.shape),
        'upblock3_resnet0_concat_input_shape': list(concat0.shape),
        'upblock3_resnet0_output_shape': list(entry_official.shape),
        ...
        'previous_upblock2_local_compare': previous_upblock2_compare,
        ...
        'next_module_preview': next_module_name(up3),
        'markers': {
            'TADSR_UNET_UPBLOCK3_RESNET0_ORACLE_TENSORS': 'PASS',
            'TADSR_UNET_UPBLOCK3_RESNET0_EFFECTIVE_WEIGHTS': 'PASS',
            'TADSR_UNET_UPBLOCK3_RESNET0_RESIDUAL_CONTRACT_ORACLE': 'PASS',
        },
    }
    META_JSON.write_text(json.dumps(metadata, indent=2), encoding='utf-8')
    SUMMARY_TXT.write_text('\n'.join([
        '# TADSR UNet up_blocks.3.resnets.0 PyTorch oracle',
        '',
        *[f'{k}: {v}' for k, v in metadata['markers'].items()],
        '',
        f"residual source: {metadata['upblock3_resnet0_consumed_residual_source']}",
        f"hidden input shape: {metadata['upblock3_resnet0_hidden_input_shape']}",
        f"residual shape: {metadata['upblock3_resnet0_residual_input_shape']}",
        f"concat shape: {metadata['upblock3_resnet0_concat_input_shape']}",
        f"output shape: {metadata['upblock3_resnet0_output_shape']}",
