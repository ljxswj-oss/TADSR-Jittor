...
ORACLE_DIR = OUT_DIR / 'oracle_tensors_unet_downblock2_resnet0'
META_JSON = ORACLE_DIR / 'unet_downblock2_resnet0_oracle_metadata.json'
SUMMARY_TXT = ORACLE_DIR / 'oracle_summary.txt'
EFFECTIVE_WEIGHTS = OUT_DIR / 'converted_unet_downblock2_resnet0_effective_weights.npz'
PREV_DOWNBLOCK1_ORACLE = OUT_DIR / 'oracle_tensors_unet_downblock1_downsampler' / 'local_downblock1_hidden_states_output.npy'
...
def main() -> int:
    ...
    with torch.no_grad():
        ...
        x_shape = list(block1_hidden.shape)
        ...
        entry_inter = resnet_manual(resnet0, block1_hidden, time_embedding)
        entry_official = resnet0(block1_hidden, time_embedding)

    saved = {}
    save_tensor(saved, 'synthetic_downblock2_resnet0_input', synthetic_x)
    save_tensor(saved, 'synthetic_downblock2_resnet0_temb', synthetic_temb)
    for k, v in synthetic_inter.items():
        save_tensor(saved, f'synthetic_downblock2_resnet0_{k}_output' if k != 'output' else 'synthetic_downblock2_resnet0_manual_output', v)
    save_tensor(saved, 'synthetic_downblock2_resnet0_output', synthetic_official)
    ...
    save_tensor(saved, 'entry_downblock2_resnet0_input', block1_hidden)
    save_tensor(saved, 'entry_downblock2_resnet0_temb', time_embedding)
    for k, v in entry_inter.items():
        save_tensor(saved, f'entry_downblock2_resnet0_{k}_output' if k != 'output' else 'entry_downblock2_resnet0_manual_output', v)
    save_tensor(saved, 'entry_downblock2_resnet0_output', entry_official)

    effective = {}
    effective_meta = {}
    prefix = 'down_blocks_2_resnets_0'
    ...
    previous_downblock1_compare = {'previous_downblock1_oracle_tensor_available': False}
    if PREV_DOWNBLOCK1_ORACLE.exists():
        prev = np.load(PREV_DOWNBLOCK1_ORACLE)
        curr = to_np(block1_hidden)
        previous_downblock1_compare = {
            'previous_downblock1_oracle_tensor_available': True,
    ...
    metadata = {
        ...
        'resnet0_input_shape': list(block1_hidden.shape),
        ...
        'previous_downblock1_compare': previous_downblock1_compare,
        'markers': {
            'TADSR_UNET_DOWNBLOCK2_RESNET0_ORACLE_TENSORS': 'PASS',
            'TADSR_UNET_DOWNBLOCK2_RESNET0_EFFECTIVE_WEIGHTS': 'PASS',
        },
        'scope_boundaries': {
            'ported_this_stage': 'down_blocks.2.resnets.0',
            'not_ported_this_stage': ['down_blocks.2.attentions.0', 'down_blocks.2.resnets.1', 'down_blocks.2.attentions.1', 'down_blocks.2.downsamplers.0', 'mid_block', 'up_blocks', 'full UNet forward'],
        },
    }
    META_JSON.write_text(json.dumps(metadata, indent=2), encoding='utf-8')
    SUMMARY_TXT.write_text(
        '\n'.join([
            '# TADSR UNet down_blocks.2.resnets.0 oracle export',
            '',
            'TADSR_UNET_DOWNBLOCK2_RESNET0_ORACLE_TENSORS: PASS',
            'TADSR_UNET_DOWNBLOCK2_RESNET0_EFFECTIVE_WEIGHTS: PASS',
            '',
            ...
            f'previous downblock1 compare: {previous_downblock1_compare}',
        ]) + '\n',
        encoding='utf-8',
    )
