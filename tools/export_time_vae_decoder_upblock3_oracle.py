...
PREV_UPBLOCK2_ORACLE = BASE_OUT / 'oracle_tensors_decoder_upblock2'
AUDIT_PATH = BASE_OUT / 'audit_time_vae_decoder_upblock3.json'
...
def load_audit_shape() -> tuple[int, int, int, int]:
    if AUDIT_PATH.exists():
        audit = json.loads(AUDIT_PATH.read_text(encoding='utf-8'))
        shape = audit.get('input_shape')
...
def export_upblock3_path(out, arrays, stats, prefix, source_label, upblock3, hidden, temb):
    res_outputs, pre_upsampler, upsampler_outputs, direct = run_upblock_parts(upblock3, hidden, temb)
    for i, value in enumerate(res_outputs):
        save(out, arrays, f'{prefix}_decoder_upblock3_resnet{i}_output', to_np(value), f'decoder.up_blocks.3.resnets.{i}({source_label})', stats)
    save(out, arrays, f'{prefix}_decoder_upblock3_pre_upsampler_output', to_np(pre_upsampler), f'decoder.up_blocks.3.pre_upsampler({source_label})', stats)
    for i, value in enumerate(upsampler_outputs):
        save(out, arrays, f'{prefix}_decoder_upblock3_upsampler{i}_output', to_np(value), f'decoder.up_blocks.3.upsamplers.{i}({source_label})', stats)
    save(out, arrays, f'{prefix}_decoder_upblock3_output', to_np(direct), f'decoder.up_blocks.3({source_label})', stats)
    return direct
...
def main() -> int:
    ...
    out = BASE_OUT / 'oracle_tensors_decoder_upblock3'
    ...
    upblock2 = dec.up_blocks[2]
    upblock3 = dec.up_blocks[3]
    ...
    with torch.no_grad():
        ...
        # Path A: isolated synthetic hidden -> decoder.up_blocks.3.
        synthetic_shape = load_audit_shape()
        ...
        save(out, arrays, 'synthetic_decoder_upblock3_input', synthetic_hidden_np, 'path_a.synthetic_linspace_hidden', stats)
        export_upblock3_path(out, arrays, stats, 'synthetic', 'synthetic', upblock3, synthetic_hidden, decoder_temb)

        # Path B: synthetic latent -> post_quant_conv -> decoder.conv_in -> mid -> upblock0 -> upblock1 -> upblock2 -> upblock3.
        prev_z = PREV_UPBLOCK2_ORACLE / 'entry_synthetic_latent_z.npy'
        if prev_z.exists():
        ...
        save(out, arrays, 'entry_decoder_upblock1_output', to_np(entry_up1), 'decoder.up_blocks.1(entry)', stats)
        save(out, arrays, 'entry_decoder_upblock2_output', to_np(entry_up2), 'decoder.up_blocks.2(entry)', stats)
        export_upblock3_path(out, arrays, stats, 'entry', 'entry', upblock3, entry_up2, decoder_temb)

        # Path C: real encoder deterministic path -> decoder.up_blocks.3.
        prev_img = PREV_UPBLOCK2_ORACLE / 'real_input_image.npy'
        if prev_img.exists():
        ...
        save(out, arrays, 'real_decoder_upblock1_output', to_np(real_up1), 'decoder.up_blocks.1(path_c)', stats)
        save(out, arrays, 'real_decoder_upblock2_output', to_np(real_up2), 'decoder.up_blocks.2(path_c)', stats)
        export_upblock3_path(out, arrays, stats, 'real', 'path_c', upblock3, real_up2, decoder_temb)

    prev_diff = None
    prev_real_up2 = PREV_UPBLOCK2_ORACLE / 'real_decoder_upblock2_output.npy'
    if prev_real_up2.exists():
        prev = np.load(prev_real_up2).astype(np.float32)
        prev_diff = float(np.max(np.abs(prev - arrays['real_decoder_upblock2_output'])))

    np.savez_compressed(out / 'time_vae_decoder_upblock3_inputs.npz', **{
        k: arrays[k] for k in [
            'synthetic_decoder_upblock3_input',
            'entry_synthetic_latent_z',
            'entry_decoder_upblock2_output',
            'real_input_image',
            'real_encoder_temb',
            'real_moments',
            'real_decoder_upblock2_output',
        ]
    })
    np.savez_compressed(out / 'time_vae_decoder_upblock3_outputs.npz', **{
        k: v for k, v in arrays.items() if k.endswith('_output') or k in {'real_latent_dist_mode'}
    })

    resnet_shortcuts = []
    for i, resnet in enumerate(upblock3.resnets):
        shortcut = getattr(resnet, 'conv_shortcut', None)
        resnet_shortcuts.append({
            'index': i,
            'conv_shortcut_exists': shortcut is not None,
            'conv_shortcut_weight_shape': list(shortcut.weight.shape) if shortcut is not None else None,
    ...
    meta = {
        ...
        'decoder_upblock3_class': f'{upblock3.__class__.__module__}.{upblock3.__class__.__name__}',
        'uses_sampling': False,
        'decoder_side_temb_is_none': True,
        'upblock3_resnet_count': len(upblock3.resnets),
        'upblock3_upsampler_count': len(upblock3.upsamplers or []),
        'upblock3_has_upsampler': bool(upblock3.upsamplers),
        'upblock3_upsampler0_status': 'PASS' if bool(upblock3.upsamplers) else 'NOT_APPLICABLE',
        'upblock3_attention_count': len(getattr(upblock3, 'attentions', []) or []),
        'upblock3_module_order': [f'resnets.{i}' for i in range(len(upblock3.resnets))] + [f'upsamplers.{i}' for i in range(len(upblock3.upsamplers or []))],
        'upblock3_input_shape': list(arrays['real_decoder_upblock2_output'].shape),
        'upblock3_output_shape': list(arrays['real_decoder_upblock3_output'].shape),
        'upblock3_channel_change': arrays['real_decoder_upblock2_output'].shape[1] != arrays['real_decoder_upblock3_output'].shape[1],
        'upblock3_shortcut_modules': resnet_shortcuts,
        ...
        'real_decoder_upblock2_output_shape': list(arrays['real_decoder_upblock2_output'].shape),
        'real_decoder_upblock3_output_shape': list(arrays['real_decoder_upblock3_output'].shape),
        'max_abs_diff_between_real_decoder_upblock2_output_and_previous_decoder_upblock2_oracle': prev_diff,
        'tensor_stats': stats,
    }
    (out / 'decoder_upblock3_oracle_metadata.json').write_text(json.dumps(meta, indent=2), encoding='utf-8')
    lines = [
        '# TimeAware VAE Decoder UpBlock3 Oracle Export',
        '',
        f"Status: **{meta['status']}**",
        '',
        f"Decoder upblock3 class: `{meta['decoder_upblock3_class']}`",
        f"Module order: `{meta['upblock3_module_order']}`",
        f"ResNet count: `{meta['upblock3_resnet_count']}`",
        f"Upsampler count: `{meta['upblock3_upsampler_count']}`",
        f"Upsampler0 status: `{meta['upblock3_upsampler0_status']}`",
        f"Channel change: `{meta['upblock3_channel_change']}`",
        f"Shortcuts: `{meta['upblock3_shortcut_modules']}`",
        f"Decoder-side temb is None: `{meta['decoder_side_temb_is_none']}`",
        f"Previous decoder-upblock2 real output diff: `{prev_diff}`",
        '',
        '## Paths',
        '',
        '- Path A: synthetic hidden -> decoder.up_blocks.3.',
        '- Path B: synthetic latent -> post_quant_conv -> decoder.conv_in -> decoder.mid_block -> decoder.up_blocks.0 -> decoder.up_blocks.1 -> decoder.up_blocks.2 -> decoder.up_blocks.3.',
        '- Path C: synthetic image -> encoder -> quant_conv -> posterior mode -> post_quant_conv -> decoder.conv_in -> decoder.mid_block -> decoder.up_blocks.0 -> decoder.up_blocks.1 -> decoder.up_blocks.2 -> decoder.up_blocks.3.',
    ]
