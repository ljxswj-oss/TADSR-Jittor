...
ORACLE_DIR = OUT_DIR / 'oracle_tensors_unet_downblock0_attention0'
META_JSON = ORACLE_DIR / 'unet_downblock0_attention0_oracle_metadata.json'
SUMMARY_TXT = ORACLE_DIR / 'oracle_summary.txt'
EFFECTIVE_WEIGHTS = OUT_DIR / 'converted_unet_downblock0_attention0_effective_weights.npz'
...
def save_tensor(tensors, name, value):
    arr = to_np(value)
    np.save(ORACLE_DIR / f'{name}.npy', arr)
    tensors[name] = stats(arr)
    return value
...
def manual_transformer_attention0(attention0, hidden_states, encoder_hidden_states):
    tensors = {}
    tb = attention0.transformer_blocks[0]
    residual = hidden_states
    norm = attention0.norm(hidden_states)
    tensors['norm'] = norm
    b, c, h, w = norm.shape
    seq = norm.permute(0, 2, 3, 1).reshape(b, h * w, c)
    tensors['sequence_input'] = seq
    proj_in = attention0.proj_in(seq)
    ...
    proj_out_seq = attention0.proj_out(x)
...
def main() -> int:
    ...
    h_shape = [1, int(attention0.in_channels), 32, 32]
    ...
    with torch.no_grad():
        synthetic_manual = manual_transformer_attention0(attention0, synthetic_hidden, encoder_hidden_states)
        ...
        entry_manual = manual_transformer_attention0(attention0, resnet0_out, encoder_hidden_states)
        entry_official = attention0(resnet0_out, encoder_hidden_states=encoder_hidden_states, return_dict=False)[0]

    save_tensor(saved, 'synthetic_attention0_input', synthetic_hidden)
    save_tensor(saved, 'synthetic_attention0_encoder_hidden_states', encoder_hidden_states)
    for k, v in synthetic_manual.items():
        save_tensor(saved, f'synthetic_attention0_{k}', v)
    save_tensor(saved, 'synthetic_attention0_official_output', synthetic_official)
    ...
    save_tensor(saved, 'entry_attention0_encoder_hidden_states', encoder_hidden_states)
    save_tensor(saved, 'entry_downblock0_resnet0_output', resnet0_out)
    for k, v in entry_manual.items():
        save_tensor(saved, f'entry_downblock0_attention0_{k}', v)
    save_tensor(saved, 'entry_downblock0_attention0_official_output', entry_official)

    arrays = {}
    emeta = {}
    prefix = 'down_blocks_0_attentions_0'
    ...
    metadata = {
        ...
        'attention0_config': {
            ...
            'out_channels': int(attention0.out_channels),
            ...
            'transformer_blocks': len(attention0.transformer_blocks),
        ...
        'transformer0_config': {
            ...
            'attn1_scale': float(tb.attn1.scale),
            'attn2_scale': float(tb.attn2.scale),
            'dropout': 0.0,
            'ff_activation': type(tb.ff.net[0]).__name__,
            'ff_inner_dim': int(tb.ff.net[2].weight.shape[1]),
            'chunk_size': getattr(tb, '_chunk_size', None),
        },
        'path_a_isolated_attention0': {'status': 'PASS', 'input_shape': h_shape, 'encoder_hidden_states_shape': encoder_shape, 'output_shape': list(synthetic_official.shape)},
        'path_b_entry_resnet0_attention0': {'status': 'PASS', 'sample_shape': sample_shape, 'resnet0_output_shape': list(resnet0_out.shape), 'encoder_hidden_states_shape': encoder_shape, 'output_shape': list(entry_official.shape)},
        ...
        'saved_tensors': saved,
        'markers': {
            'TADSR_UNET_DOWNBLOCK0_ATTENTION0_ORACLE_TENSORS': 'PASS',
            'TADSR_UNET_DOWNBLOCK0_ATTENTION0_EFFECTIVE_WEIGHTS': 'PASS',
        },
        'not_in_scope': ['full down_blocks.0', 'resnets.1', 'attentions.1', 'downsampler', 'full UNet forward', 'full TADSR inference'],
    }
    META_JSON.write_text(json.dumps(metadata, indent=2, default=str), encoding='utf-8')
    lines = [
        '# TADSR UNet down_blocks.0.attentions.0 Oracle Export', '',
        'TADSR_UNET_DOWNBLOCK0_ATTENTION0_ORACLE_TENSORS: PASS',
        'TADSR_UNET_DOWNBLOCK0_ATTENTION0_EFFECTIVE_WEIGHTS: PASS', '',
        f"Oracle dir: `{ORACLE_DIR}`",
        f"Effective weights: `{EFFECTIVE_WEIGHTS}`",
        f"Path A output shape: `{metadata['path_a_isolated_attention0']['output_shape']}`",
        f"Path B output shape: `{metadata['path_b_entry_resnet0_attention0']['output_shape']}`",
        ...
        '',
        'Attention probability tensors were intentionally not saved to avoid large artifacts; Q/K/V and attention outputs are exported.',
    ]
    SUMMARY_TXT.write_text('\n'.join(lines) + '\n', encoding='utf-8')
    print('TADSR_UNET_DOWNBLOCK0_ATTENTION0_ORACLE_TENSORS: PASS')
    print('TADSR_UNET_DOWNBLOCK0_ATTENTION0_EFFECTIVE_WEIGHTS: PASS')
