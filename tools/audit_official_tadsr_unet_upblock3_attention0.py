...
OUT_JSON = OUT_DIR / 'audit_tadsr_unet_upblock3_attention0.json'
OUT_TXT = OUT_DIR / 'audit_tadsr_unet_upblock3_attention0.txt'
...
def main() -> int:
    ...
    cfg = attention_config(attention0, 'up_blocks_3_attentions_0', 'unet.up_blocks.3.attentions.0')
    transformer_blocks = list(getattr(attention0, 'transformer_blocks', []) or [])
    if len(transformer_blocks) != 1:
        status = 'BLOCKED_NEEDS_SPLIT'
    else:
        status = 'PASS'
    ...
    if tb0 is not None:
        ...
        head_dim = int(cfg.get('attention_head_dim') or 0)
        inner_dim = int(cfg.get('inner_dim') or 0)
    ...
    markers = {
        'TADSR_UNET_UPBLOCK3_ATTENTION0_AUDIT': status,
        'TADSR_UNET_UPBLOCK3_ATTENTION0_TRANSFORMER_AUDIT': status,
        'TADSR_UNET_UPBLOCK3_ATTENTION0_LORA_AUDIT': 'PASS' if status == 'PASS' else status,
        'TADSR_UNET_UPBLOCK3_ATTENTION0_RESIDUAL_CONTRACT_AUDIT': 'PASS' if status == 'PASS' else status,
    }
    result = {
        'status': status,
        'scope': 'up_blocks.3.attentions.0 audit only; up_blocks.3.resnets.1 and later modules remain intentionally unopened.',
        ...
        'upblock3_overview': {
            ...
            'known_completed_modules_before_this_stage': ['full local up_blocks.2', 'up_blocks.3.resnets.0'],
            'target_module': 'up_blocks.3.attentions.0',
            'actual_next_module_after_attention0': next_module,
        ...
        'entry_context_shapes': {
            ...
            'upblock3_resnet0_residual_shape': list(res0_hidden.shape),
            'upblock3_resnet0_output': list(resnet0_out.shape),
            'upblock3_attention0_input': list(resnet0_out.shape),
            'upblock3_attention0_output': list(attention0_out.shape),
            'remaining_residual_count_after_resnet0_pop': len(remaining_after_resnet0_local),
            'remaining_residual_count_after_attention0': len(remaining_after_resnet0_local),
        ...
        'existing_tester_compatibility': {
            ...
            'reason': 'Same Transformer2DModel + BasicTransformerBlock pattern; selected with prefix=up_blocks_3_attentions_0.',
        },
    ...
    OUT_TXT.write_text('\n'.join([
        '# TADSR UNet up_blocks.3.attentions.0 official audit',
        '',
        *[f'{k}: {v}' for k, v in result['markers'].items()],
        '',
        f"target: {result['upblock3_overview']['target_module']}",
        f"next unopened module: {result['upblock3_overview']['actual_next_module_after_attention0']}",
        f"attention input shape: {result['entry_context_shapes']['upblock3_attention0_input']}",
        f"attention output shape: {result['entry_context_shapes']['upblock3_attention0_output']}",
    ]) + '\n', encoding='utf-8')
