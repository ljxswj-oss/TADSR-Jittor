...
ORACLE_DIR = OUT_DIR / 'oracle_tensors_unet_upblock2_attention1'
META_JSON = ORACLE_DIR / 'unet_upblock2_attention1_oracle_metadata.json'
SUMMARY_TXT = ORACLE_DIR / 'oracle_summary.txt'
EFFECTIVE_WEIGHTS = OUT_DIR / 'converted_unet_upblock2_attention1_effective_weights.npz'
PREV_UPBLOCK2_RESNET1_ORACLE = OUT_DIR / 'oracle_tensors_unet_upblock2_resnet1' / 'entry_upblock2_resnet1_output.npy'
...
def main() -> int:
    ...
    saved = {}
    for name, tensor in [
        ('synthetic_upblock2_attention1_input', synthetic_hidden),
        ('synthetic_upblock2_attention1_encoder_hidden_states', encoder_hidden_states),
    ]:
        save_tensor(saved, name, tensor)
    for k, v in synthetic_manual.items():
        save_tensor(saved, f'synthetic_upblock2_attention1_{k}', v)
    save_tensor(saved, 'synthetic_upblock2_attention1_official_output', synthetic_official)
    ...
    for name, tensor in [
        ...
        ('entry_upblock2_attention1_input', resnet1_out),
    ]:
        save_tensor(saved, name, tensor)
    for k, v in entry_manual.items():
        save_tensor(saved, f'entry_upblock2_attention1_{k}', v)
    save_tensor(saved, 'entry_upblock2_attention1_official_output', entry_official)

    arrays = {}
    emeta = {}
    prefix = 'up_blocks_2_attentions_1'
    ...
    previous_resnet1_compare = {'previous_upblock2_resnet1_oracle_tensor_available': False}
    if PREV_UPBLOCK2_RESNET1_ORACLE.exists():
        prev = np.load(PREV_UPBLOCK2_RESNET1_ORACLE)
        curr = to_np(resnet1_out)
        previous_resnet1_compare = {
            'previous_upblock2_resnet1_oracle_tensor_available': True,
    ...
    metadata = {
        ...
        'actual_next_module_after_upblock2_attention1': 'up_blocks.2.resnets.2',
        'attention_config': {
            'prefix': prefix,
        ...
        'previous_upblock2_resnet1_compare': previous_resnet1_compare,
        ...
        'markers': {
            'TADSR_UNET_UPBLOCK2_ATTENTION1_ORACLE_TENSORS': 'PASS',
            'TADSR_UNET_UPBLOCK2_ATTENTION1_EFFECTIVE_WEIGHTS': 'PASS',
            'TADSR_UNET_UPBLOCK2_ATTENTION1_RESIDUAL_CONTRACT_ORACLE': 'PASS',
        },
    }
    META_JSON.write_text(json.dumps(metadata, indent=2), encoding='utf-8')
    SUMMARY_TXT.write_text('\n'.join([
        '# TADSR UNet up_blocks.2.attentions.1 PyTorch oracle',
        '',
        *[f'{k}: {v}' for k, v in metadata['markers'].items()],
        ...
        f"next unopened module: {metadata['actual_next_module_after_upblock2_attention1']}",
    ]) + '\n', encoding='utf-8')
