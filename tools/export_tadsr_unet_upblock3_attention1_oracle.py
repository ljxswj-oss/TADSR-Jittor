...
ORACLE_DIR = OUT_DIR / 'oracle_tensors_unet_upblock3_attention1'
META_JSON = ORACLE_DIR / 'unet_upblock3_attention1_oracle_metadata.json'
SUMMARY_TXT = ORACLE_DIR / 'oracle_summary.txt'
EFFECTIVE_WEIGHTS = OUT_DIR / 'converted_unet_upblock3_attention1_effective_weights.npz'
PREV_UPBLOCK3_RESNET1_ORACLE = OUT_DIR / 'oracle_tensors_unet_upblock3_resnet1' / 'entry_upblock3_resnet1_output.npy'
...
def main() -> int:
    ...
    saved = {}
    for name, tensor in [
        ('synthetic_upblock3_attention1_input', synthetic_hidden),
        ('synthetic_upblock3_attention1_encoder_hidden_states', encoder_hidden_states),
    ]:
        save_tensor(saved, name, tensor)
    for key, tensor in synthetic_manual.items():
        save_tensor(saved, f'synthetic_upblock3_attention1_{key}', tensor)
    save_tensor(saved, 'synthetic_upblock3_attention1_official_output', synthetic_official)
    ...
    for key, tensor in entry_manual.items():
        save_tensor(saved, f'entry_upblock3_attention1_{key}', tensor)
    save_tensor(saved, 'entry_upblock3_attention1_official_output', entry_official)

    arrays = {}
    emeta = {}
    prefix = 'up_blocks_3_attentions_1'
    ...
    emeta[f'{prefix}_norm'] = {
        ...
        'weight_shape': list(attention1.norm.weight.shape),
        'bias_shape': list(attention1.norm.bias.shape),
    }
    ...
    previous_resnet1_compare = {'previous_upblock3_resnet1_oracle_tensor_available': False}
    if PREV_UPBLOCK3_RESNET1_ORACLE.exists():
        prev = np.load(PREV_UPBLOCK3_RESNET1_ORACLE)
        curr = to_np(resnet1_out)
        previous_resnet1_compare = {
            'previous_upblock3_resnet1_oracle_tensor_available': True,
    ...
    metadata = {
        ...
        'upblock3_attention1_input_shape': list(resnet1_out.shape),
        'upblock3_attention1_output_shape': list(entry_official.shape),
        ...
        'remaining_residual_count_after_upblock3_attention1': len(remaining_after_resnet1_local),
        'attention1_consumes_accumulated_residuals': False,
        'actual_next_module_after_upblock3_attention1': next_module,
        'attention_config': {
            'prefix': prefix,
        ...
        'transformer0_config': {
            'heads': int(attention1.num_attention_heads),
            'head_dim': int(attention1.attention_head_dim),
            'inner_dim': int(attention1.num_attention_heads) * int(attention1.attention_head_dim),
        ...
        'previous_upblock3_resnet1_compare': previous_resnet1_compare,
        ...
        'markers': {
            'TADSR_UNET_UPBLOCK3_ATTENTION1_ORACLE_TENSORS': 'PASS',
            'TADSR_UNET_UPBLOCK3_ATTENTION1_EFFECTIVE_WEIGHTS': 'PASS',
            'TADSR_UNET_UPBLOCK3_ATTENTION1_RESIDUAL_CONTRACT_ORACLE': 'PASS',
        },
    }
    META_JSON.write_text(json.dumps(metadata, indent=2), encoding='utf-8')
    SUMMARY_TXT.write_text('\n'.join([
        '# TADSR UNet up_blocks.3.attentions.1 PyTorch oracle',
        '',
        *[f'{k}: {v}' for k, v in metadata['markers'].items()],
        ...
        f"next unopened module: {metadata['actual_next_module_after_upblock3_attention1']}",
    ]) + '\n', encoding='utf-8')
