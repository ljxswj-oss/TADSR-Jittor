...
ORACLE_DIR = OUT_DIR / 'oracle_tensors_unet_upblock2_attention0'
META_JSON = ORACLE_DIR / 'unet_upblock2_attention0_oracle_metadata.json'
SUMMARY_TXT = ORACLE_DIR / 'oracle_summary.txt'
EFFECTIVE_WEIGHTS = OUT_DIR / 'converted_unet_upblock2_attention0_effective_weights.npz'
PREV_UPBLOCK2_RESNET0_ORACLE = OUT_DIR / 'oracle_tensors_unet_upblock2_resnet0' / 'entry_upblock2_resnet0_output.npy'
...
def main() -> int:
    ...
    saved = {}
    for name, tensor in [
        ('synthetic_upblock2_attention0_input', synthetic_hidden),
        ('synthetic_upblock2_attention0_encoder_hidden_states', encoder_hidden_states),
    ]:
        save_tensor(saved, name, tensor)
    for k, v in synthetic_manual.items():
        save_tensor(saved, f'synthetic_upblock2_attention0_{k}', v)
    save_tensor(saved, 'synthetic_upblock2_attention0_official_output', synthetic_official)
    ...
    for name, tensor in [
        ...
        ('entry_upblock2_attention0_input', resnet0_out),
    ]:
        save_tensor(saved, name, tensor)
    for k, v in entry_manual.items():
        save_tensor(saved, f'entry_upblock2_attention0_{k}', v)
    save_tensor(saved, 'entry_upblock2_attention0_official_output', entry_official)

    arrays = {}
    emeta = {}
    prefix = 'up_blocks_2_attentions_0'
    ...
    previous_resnet0_compare = {'previous_upblock2_resnet0_oracle_tensor_available': False}
    if PREV_UPBLOCK2_RESNET0_ORACLE.exists():
        prev = np.load(PREV_UPBLOCK2_RESNET0_ORACLE)
        curr = to_np(resnet0_out)
        previous_resnet0_compare = {
            'previous_upblock2_resnet0_oracle_tensor_available': True,
    ...
    metadata = {
        ...
        'actual_next_module_after_upblock2_attention0': 'up_blocks.2.resnets.1',
        'attention_config': {
            'prefix': prefix,
        ...
        'previous_upblock2_resnet0_compare': previous_resnet0_compare,
        ...
        'markers': {
            'TADSR_UNET_UPBLOCK2_ATTENTION0_ORACLE_TENSORS': 'PASS',
            'TADSR_UNET_UPBLOCK2_ATTENTION0_EFFECTIVE_WEIGHTS': 'PASS',
            'TADSR_UNET_UPBLOCK2_ATTENTION0_RESIDUAL_CONTRACT_ORACLE': 'PASS',
        },
    }
    META_JSON.write_text(json.dumps(metadata, indent=2), encoding='utf-8')
    SUMMARY_TXT.write_text('\n'.join([
        '# TADSR UNet up_blocks.2.attentions.0 PyTorch oracle',
        '',
        *[f'{k}: {v}' for k, v in metadata['markers'].items()],
        ...
        f"next unopened module: {metadata['actual_next_module_after_upblock2_attention0']}",
    ]) + '\n', encoding='utf-8')
