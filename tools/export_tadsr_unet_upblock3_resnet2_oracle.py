...
ORACLE_DIR = OUT_DIR / 'oracle_tensors_unet_upblock3_resnet2'
META_JSON = ORACLE_DIR / 'unet_upblock3_resnet2_oracle_metadata.json'
SUMMARY_TXT = ORACLE_DIR / 'oracle_summary.txt'
EFFECTIVE_WEIGHTS = OUT_DIR / 'converted_unet_upblock3_resnet2_effective_weights.npz'
PREV_ATTENTION1_ORACLE = OUT_DIR / 'oracle_tensors_unet_upblock3_attention1' / 'entry_upblock3_attention1_output.npy'
...
def main() -> int:
    ...
    if len(getattr(up3, 'resnets', []) or []) < 3:
        raise RuntimeError('Official up_blocks.3 must expose resnets.2 for this stage.')
    if len(getattr(up3, 'attentions', []) or []) < 2:
        raise RuntimeError('Official up_blocks.3 must expose attentions.1 before resnets.2.')
    ...
    with torch.no_grad():
        ...
        entry_inter = resnet_manual(resnet2, concat2, ctx['time_embedding'])
        entry_official = resnet2(concat2, ctx['time_embedding'])

        h_shape = list(attention1_out.shape)
    ...
    saved = {}
    for name, tensor in [
        ('synthetic_upblock3_resnet2_hidden_input', synthetic_hidden),
        ('synthetic_upblock3_resnet2_res_hidden', synthetic_res),
        ('synthetic_upblock3_resnet2_concat_input', synthetic_concat),
        ('synthetic_upblock3_resnet2_temb', synthetic_temb),
    ]:
        save_tensor(saved, name, tensor)
    save_resnet_intermediates(saved, 'synthetic_upblock3_resnet2', synthetic_inter, synthetic_official)
    ...
    for name, tensor in [
        ...
        ('entry_upblock3_attention1_input', resnet1_out),
        ...
        ('entry_upblock3_resnet2_temb', ctx['time_embedding']),
    ]:
        save_tensor(saved, name, tensor)
    save_resnet_intermediates(saved, 'entry_upblock3_resnet2', entry_inter, entry_official)

    effective = {}
    effective_meta = {}
    prefix = 'up_blocks_3_resnets_2'
    ...
    previous_attention1_compare = {'previous_upblock3_attention1_oracle_tensor_available': False}
    if PREV_ATTENTION1_ORACLE.exists():
        prev = np.load(PREV_ATTENTION1_ORACLE)
        curr = to_np(attention1_out)
        previous_attention1_compare = {
            'previous_upblock3_attention1_oracle_tensor_available': True,
    ...
    metadata = {
        ...
        'upblock3_attention1_consumes_accumulated_residuals': False,
        'upblock3_resnet2_consumed_residual_index_in_accumulated_tuple': res2_global_index,
        'upblock3_resnet2_consumed_residual_current_remaining_tuple_index': len(remaining_after_resnet1_local) - 1,
        ...
        'upblock3_resnet2_hidden_input_shape': list(attention1_out.shape),
        'upblock3_resnet2_residual_input_shape': list(res2_hidden.shape),
        'upblock3_resnet2_concat_input_shape': list(concat2.shape),
        'upblock3_resnet2_output_shape': list(entry_official.shape),
        'remaining_residual_count_after_upblock3_resnet2_pop': len(remaining_after_resnet2_local),
        'remaining_residual_shapes_after_upblock3_resnet2_pop': [list(x.shape) for x in remaining_after_resnet2_local],
        ...
        'previous_upblock3_attention1_compare': previous_attention1_compare,
        'upblock2_local_return_info': up2_return,
        'uses_full_unet_forward': False,
        'uses_upblock3_attention2': False,
        'uses_output_tail': False,
        'uses_full_tadsr_inference': False,
        'uses_stochastic_sampling': False,
        'next_module_preview': next_name,
        'markers': {
            'TADSR_UNET_UPBLOCK3_RESNET2_ORACLE_TENSORS': 'PASS',
            'TADSR_UNET_UPBLOCK3_RESNET2_EFFECTIVE_WEIGHTS': 'PASS',
            'TADSR_UNET_UPBLOCK3_RESNET2_RESIDUAL_CONTRACT_ORACLE': 'PASS',
        },
    }
    META_JSON.write_text(json.dumps(metadata, indent=2), encoding='utf-8')
    SUMMARY_TXT.write_text('\n'.join([
        '# TADSR UNet up_blocks.3.resnets.2 PyTorch oracle',
        '',
        *[f'{k}: {v}' for k, v in metadata['markers'].items()],
        '',
        f"residual source: {metadata['upblock3_resnet2_consumed_residual_source']}",
        f"hidden input shape: {metadata['upblock3_resnet2_hidden_input_shape']}",
        f"residual shape: {metadata['upblock3_resnet2_residual_input_shape']}",
        f"concat shape: {metadata['upblock3_resnet2_concat_input_shape']}",
        f"output shape: {metadata['upblock3_resnet2_output_shape']}",
