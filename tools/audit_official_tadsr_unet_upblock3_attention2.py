...
OUT_JSON = OUT_DIR / 'audit_tadsr_unet_upblock3_attention2.json'
OUT_TXT = OUT_DIR / 'audit_tadsr_unet_upblock3_attention2.txt'
...
def main() -> int:
    ...
    cfg = attention_config(attention2, 'up_blocks_3_attentions_2', 'unet.up_blocks.3.attentions.2')
    transformer_blocks = list(getattr(attention2, 'transformer_blocks', []) or [])
    ...
    markers = {
        'TADSR_UNET_UPBLOCK3_ATTENTION2_AUDIT': status,
        'TADSR_UNET_UPBLOCK3_ATTENTION2_TRANSFORMER_AUDIT': status,
        'TADSR_UNET_UPBLOCK3_ATTENTION2_LORA_AUDIT': 'PASS' if status == 'PASS' else status,
        'TADSR_UNET_UPBLOCK3_ATTENTION2_RESIDUAL_CONTRACT_AUDIT': 'PASS' if status == 'PASS' else status,
    }
    result = {
        'status': status,
        'scope': 'up_blocks.3.attentions.2 audit only; output tail and later full-forward modules remain intentionally unopened.',
        ...
        'upblock3_overview': {
            ...
            'target_module': 'up_blocks.3.attentions.2',
            'actual_next_module_after_attention2': next_module,
            'remaining_modules_after_this_stage': ['output_tail'],
        ...
        'entry_context_shapes': {
            ...
            'upblock3_resnet2_output': list(resnet2_out.shape),
            'upblock3_attention2_input': list(resnet2_out.shape),
            'upblock3_attention2_output': list(attention2_out.shape),
            'remaining_residual_count_after_resnet1_pop': len(remaining_after_resnet1_local),
            'remaining_residual_count_after_attention2': len(remaining_after_resnet2_local),
            'attention2_consumes_accumulated_residuals': False,
            'attention_mask': None,
            'encoder_attention_mask': None,
            'cross_attention_kwargs': None,
        },
        'attention2_input_stats': stats_tensor(resnet2_out),
        'attention2_output_stats': stats_tensor(attention2_out),
        ...
        'existing_tester_compatibility': {
            ...
            'reason': 'Same Transformer2DModel + BasicTransformerBlock pattern; selected with prefix=up_blocks_3_attentions_2.',
        },
        'lora_modules': lora_modules,
        'residual_contract': {
            'status': 'PASS' if status == 'PASS' else status,
            'attention2_consumes_accumulated_residuals': False,
            'remaining_residual_count_before_attention2': len(remaining_after_resnet2_local),
            'remaining_residual_count_after_attention2': len(remaining_after_resnet2_local),
            'remaining_tuple_reserved_for': 'none; up_blocks.3 local residual tuple is exhausted before output tail',
        },
        'lora_audit': {
            'status': 'PASS' if status == 'PASS' else status,
    ...
    OUT_TXT.write_text('\n'.join([
        '# TADSR UNet up_blocks.3.attentions.2 official audit',
        '',
        *[f'{k}: {v}' for k, v in result['markers'].items()],
        '',
        f"target: {result['upblock3_overview']['target_module']}",
        f"next unopened module: {result['upblock3_overview']['actual_next_module_after_attention2']}",
        f"attention input shape: {result['entry_context_shapes']['upblock3_attention2_input']}",
        f"attention output shape: {result['entry_context_shapes']['upblock3_attention2_output']}",
        'scope: up_blocks.3.attentions.2 only; output tail/full upblock/full UNet/full inference were not executed.',
    ]) + '\n', encoding='utf-8')
