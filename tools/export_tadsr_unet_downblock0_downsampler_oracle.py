...
ORACLE_DIR = OUT_DIR / 'oracle_tensors_unet_downblock0_downsampler'
META_JSON = ORACLE_DIR / 'unet_downblock0_downsampler_oracle_metadata.json'
SUMMARY_TXT = ORACLE_DIR / 'oracle_summary.txt'
EFFECTIVE_WEIGHTS = OUT_DIR / 'converted_unet_downblock0_downsampler_effective_weights.npz'
PREV_ATTENTION1_ORACLE = OUT_DIR / 'oracle_tensors_unet_downblock0_attention1' / 'entry_downblock0_attention1_official_output.npy'
...
def main() -> int:
    ...
    synthetic_shape = [1, int(downsampler.channels), 32, 32]
    ...
    with torch.no_grad():
        ...
        block_forward_out, block_res_samples = block(conv_in, time_embedding, encoder_hidden_states=encoder_hidden_states)
    save_tensor(saved, 'synthetic_downblock0_downsampler_input', synthetic_hidden)
    save_tensor(saved, 'synthetic_downblock0_downsampler_output', synthetic_output)
    ...
    save_tensor(saved, 'entry_downblock0_encoder_hidden_states', encoder_hidden_states)
    ...
    save_tensor(saved, 'entry_downblock0_attention1_output', attention1_out)
    save_tensor(saved, 'entry_downblock0_downsampler_output', downsample_out)
    save_tensor(saved, 'entry_downblock0_block_forward_output', block_forward_out)
    for i, tensor in enumerate(block_res_samples):
        save_tensor(saved, f'entry_downblock0_block_forward_res_sample_{i}', tensor)
    ...
    block_diff = (downsample_out - block_forward_out).abs()
    ...
    metadata = {
        ...
        'path_b_entry_full_downblock0': {
            'status': 'PASS',
            'sample_shape': sample_shape,
            'attention1_shape': list(attention1_out.shape),
            'output_shape': list(downsample_out.shape),
            'official_block_forward_output_shape': list(block_forward_out.shape),
            'official_block_res_sample_shapes': [list(x.shape) for x in block_res_samples],
        },
        'manual_chain_vs_official_block_forward': {
            'status': 'PASS' if float(block_diff.max()) < 1e-5 else 'FAIL',
        ...
        'saved_tensors': saved,
        'markers': {
            'TADSR_UNET_DOWNBLOCK0_DOWNSAMPLER_ORACLE_TENSORS': 'PASS',
            'TADSR_UNET_DOWNBLOCK0_DOWNSAMPLER_EFFECTIVE_WEIGHTS': 'PASS',
            'TADSR_UNET_DOWNBLOCK0_ORACLE_TENSORS': 'PASS',
        },
    ...
    lines = [
        '# TADSR UNet down_blocks.0.downsamplers.0 Oracle Export',
        '',
        'TADSR_UNET_DOWNBLOCK0_DOWNSAMPLER_ORACLE_TENSORS: PASS',
        'TADSR_UNET_DOWNBLOCK0_DOWNSAMPLER_EFFECTIVE_WEIGHTS: PASS',
        'TADSR_UNET_DOWNBLOCK0_ORACLE_TENSORS: PASS',
        ...
        f"Path B entry full down_blocks.0 output shape: `{metadata['path_b_entry_full_downblock0']['output_shape']}`",
        f"Manual chain vs official block.forward max abs error: `{metadata['manual_chain_vs_official_block_forward']['max_abs_error']}`",
        '',
        'This export completes the local `down_blocks.0` chain only. Full UNet forward and full TADSR inference remain out of scope.',
    ]
    SUMMARY_TXT.write_text('\n'.join(lines) + '\n', encoding='utf-8')
    print('TADSR_UNET_DOWNBLOCK0_DOWNSAMPLER_ORACLE_TENSORS: PASS')
    print('TADSR_UNET_DOWNBLOCK0_DOWNSAMPLER_EFFECTIVE_WEIGHTS: PASS')
    print('TADSR_UNET_DOWNBLOCK0_ORACLE_TENSORS: PASS')
    print(json.dumps({'status': 'PASS', 'metadata': str(META_JSON), 'effective_weights': str(EFFECTIVE_WEIGHTS)}, indent=2))
    return 0 if metadata['manual_chain_vs_official_block_forward']['status'] == 'PASS' else 1
