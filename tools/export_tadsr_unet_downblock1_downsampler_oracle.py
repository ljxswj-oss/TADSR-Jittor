...
ORACLE_DIR = OUT_DIR / 'oracle_tensors_unet_downblock1_downsampler'
META_JSON = ORACLE_DIR / 'unet_downblock1_downsampler_oracle_metadata.json'
SUMMARY_TXT = ORACLE_DIR / 'oracle_summary.txt'
EFFECTIVE_WEIGHTS = OUT_DIR / 'converted_unet_downblock1_downsampler_effective_weights.npz'
PREV_ATTENTION1_ORACLE = OUT_DIR / 'oracle_tensors_unet_downblock1_attention1' / 'entry_downblock1_attention1_output.npy'
...
def main() -> int:
    ...
    synthetic_shape = [1, int(downsampler.channels), 16, 16]
    ...
    with torch.no_grad():
        ...
        resnet0_out = block1.resnets[0](down0_out, time_embedding)
        attention0_out = block1.attentions[0](resnet0_out, encoder_hidden_states=encoder_hidden_states, return_dict=False)[0]
        resnet1_out = block1.resnets[1](attention0_out, time_embedding)
        ...
        block1_forward_out, block1_res_samples = block1(down0_out, time_embedding, encoder_hidden_states=encoder_hidden_states)
    save_tensor(saved, 'synthetic_downblock1_downsampler_input', synthetic_hidden)
    save_tensor(saved, 'synthetic_downblock1_downsampler_output', synthetic_output)
    ...
    save_tensor(saved, 'entry_downblock1_resnet1_output', resnet1_out)
    save_tensor(saved, 'entry_downblock1_attention1_output', attention1_out)
    save_tensor(saved, 'entry_downblock1_downsampler_output', downsample_out)
    save_tensor(saved, 'local_downblock1_hidden_states_output', block1_forward_out)
    for i, tensor in enumerate(block1_res_samples):
        save_tensor(saved, f'local_downblock1_res_sample_{i}', tensor)
    ...
    block_diff = (downsample_out - block1_forward_out).abs()
    output_state_diffs = []
    ...
    output_states_status = 'PASS' if len(block1_res_samples) == 3 and all(x['max_abs_error'] < 1e-5 for x in output_state_diffs) else 'FAIL'
    metadata = {
        ...
        'path_b_entry_full_downblock1': {
            ...
            'official_block_forward_output_shape': list(block1_forward_out.shape),
            'official_block_res_sample_shapes': [list(x.shape) for x in block1_res_samples],
        },
        ...
        'markers': {
            ...
            'TADSR_UNET_DOWNBLOCK1_ORACLE_TENSORS': 'PASS',
            'TADSR_UNET_DOWNBLOCK1_OUTPUT_STATES_ORACLE_TENSORS': output_states_status,
        },
    ...
    lines = [
        '# TADSR UNet down_blocks.1.downsamplers.0 Oracle Export',
        '',
        f"TADSR_UNET_DOWNBLOCK1_DOWNSAMPLER_ORACLE_TENSORS: {metadata['markers']['TADSR_UNET_DOWNBLOCK1_DOWNSAMPLER_ORACLE_TENSORS']}",
        f"TADSR_UNET_DOWNBLOCK1_DOWNSAMPLER_EFFECTIVE_WEIGHTS: {metadata['markers']['TADSR_UNET_DOWNBLOCK1_DOWNSAMPLER_EFFECTIVE_WEIGHTS']}",
        f"TADSR_UNET_DOWNBLOCK1_ORACLE_TENSORS: {metadata['markers']['TADSR_UNET_DOWNBLOCK1_ORACLE_TENSORS']}",
        f"TADSR_UNET_DOWNBLOCK1_OUTPUT_STATES_ORACLE_TENSORS: {metadata['markers']['TADSR_UNET_DOWNBLOCK1_OUTPUT_STATES_ORACLE_TENSORS']}",
        '',
        ...
        f"Path B entry full down_blocks.1 output shape: `{metadata['path_b_entry_full_downblock1']['output_shape']}`",
        ...
        '',
        'This export completes the local `down_blocks.1` chain only. Full UNet forward and full TADSR inference remain out of scope.',
    ]
