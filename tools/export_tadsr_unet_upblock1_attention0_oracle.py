...
ORACLE_DIR = OUT_DIR / 'oracle_tensors_unet_upblock1_attention0'
META_JSON = ORACLE_DIR / 'unet_upblock1_attention0_oracle_metadata.json'
SUMMARY_TXT = ORACLE_DIR / 'oracle_summary.txt'
EFFECTIVE_WEIGHTS = OUT_DIR / 'converted_unet_upblock1_attention0_effective_weights.npz'
PREV_UPBLOCK1_RESNET0_ORACLE = OUT_DIR / 'oracle_tensors_unet_upblock1_resnet0' / 'entry_upblock1_resnet0_output.npy'
...
def main() -> int:
    ...
    if len(up1.resnets) < 2 or len(getattr(up1, 'attentions', []) or []) < 1:
        raise RuntimeError('Official up_blocks.1 must expose attentions.0 and a following resnets.1 for this stage audit/export.')
    ...
    saved = {}
    for name, tensor in [
        ('synthetic_upblock1_attention0_input', synthetic_hidden),
        ('synthetic_upblock1_attention0_encoder_hidden_states', encoder_hidden_states),
    ]:
        save_tensor(saved, name, tensor)
    for k, v in synthetic_manual.items():
        save_tensor(saved, f'synthetic_upblock1_attention0_{k}', v)
    save_tensor(saved, 'synthetic_upblock1_attention0_official_output', synthetic_official)
    for name, tensor in [
    ...
    for k, v in entry_manual.items():
        save_tensor(saved, f'entry_upblock1_attention0_{k}', v)
    save_tensor(saved, 'entry_upblock1_attention0_official_output', entry_official)

    arrays = {}
    emeta = {}
    prefix = 'up_blocks_1_attentions_0'
    ...
    previous_resnet0_compare = {'previous_upblock1_resnet0_oracle_tensor_available': False}
    if PREV_UPBLOCK1_RESNET0_ORACLE.exists():
        prev = np.load(PREV_UPBLOCK1_RESNET0_ORACLE)
        curr = to_np(resnet0_out)
        previous_resnet0_compare = {
            'previous_upblock1_resnet0_oracle_tensor_available': True,
    ...
    metadata = {
        ...
        'path_b_entry_downblocks_midblock_upblock0_upblock1_resnet0_attention0': {'status': 'PASS', 'sample_shape': sample_shape, 'input_shape': list(resnet0_out.shape), 'output_shape': list(entry_official.shape)},
        'previous_upblock1_resnet0_compare': previous_resnet0_compare,
        'attention0_consumes_accumulated_residuals': False,
        'actual_next_module_after_upblock1_attention0': 'up_blocks.1.resnets.1',
        ...
        'scope_boundaries': {
            'ported_this_stage': 'up_blocks.1.attentions.0 only, after up_blocks.1.resnets.0',
            'not_ported_this_stage': ['up_blocks.1.resnets.1', 'up_blocks.1.attentions.1', 'up_blocks.1.resnets.2', 'up_blocks.1.attentions.2', 'up_blocks.1.upsamplers.0', 'full up_blocks.1', 'up_blocks.2', 'up_blocks.3', 'full UNet forward', 'full TADSR inference', 'generic runtime LoRA'],
        },
    }
    META_JSON.write_text(json.dumps(metadata, indent=2, default=str), encoding='utf-8')
    SUMMARY_TXT.write_text('\n'.join([
        '# TADSR UNet up_blocks.1.attentions.0 PyTorch oracle',
        '',
        *[f'{k}: {v}' for k, v in metadata['markers'].items()],
        '',
        f"input shape: {metadata['path_b_entry_downblocks_midblock_upblock0_upblock1_resnet0_attention0']['input_shape']}",
        f"output shape: {metadata['path_b_entry_downblocks_midblock_upblock0_upblock1_resnet0_attention0']['output_shape']}",
        ...
        'next unopened module: up_blocks.1.resnets.1',
