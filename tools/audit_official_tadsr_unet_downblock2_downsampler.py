...
OUT_JSON = OUT_DIR / 'audit_tadsr_unet_downblock2_downsampler.json'
OUT_TXT = OUT_DIR / 'audit_tadsr_unet_downblock2_downsampler.txt'
...
def main() -> int:
    ...
    block3 = unet.down_blocks[3] if len(unet.down_blocks) > 3 else None
    if getattr(block2, 'downsamplers', None) is None or len(block2.downsamplers) != 1:
        raise RuntimeError(f'Expected exactly one downsampler in down_blocks.2, got {getattr(block2, "downsamplers", None)}')
    ...
    with torch.no_grad():
        ...
        down1_out, down1_res_samples = block1(down0_out, temb, encoder_hidden_states=encoder_hidden_states)
        resnet0_out = block2.resnets[0](down1_out, temb)
        attention0_out = block2.attentions[0](resnet0_out, encoder_hidden_states=encoder_hidden_states, return_dict=False)[0]
        resnet1_out = block2.resnets[1](attention0_out, temb)
        ...
        block2_out, block2_res_samples = block2(down1_out, temb, encoder_hidden_states=encoder_hidden_states)
        diff = (downsample_out - block2_out).abs()
    ...
    output_state_pass = len(block2_res_samples) == 3 and all(x['max_abs_error'] < 1e-5 for x in output_state_diffs)
    ...
    metadata = {
        ...
        'source_module_path': 'unet.down_blocks.2.downsamplers.0',
        'block_class': f'{type(block2).__module__}.{type(block2).__name__}',
        'block_forward_signature': str(inspect.signature(block2.forward)),
        ...
        'dry_run': {
            ...
            'downblock1_output': stats_tensor(down1_out),
            ...
            'block_forward_output': stats_tensor(block2_out),
            'block_forward_res_sample_shapes': [list(x.shape) for x in block2_res_samples],
        ...
        'down_blocks3_preview': {
            'exists': block3 is not None,
            'class': f'{type(block3).__module__}.{type(block3).__name__}' if block3 is not None else None,
            'resnet_count': len(getattr(block3, 'resnets', [])) if block3 is not None else 0,
            'attention_count': len(getattr(block3, 'attentions', [])) if block3 is not None and getattr(block3, 'attentions', None) is not None else 0,
            'downsampler_count': len(getattr(block3, 'downsamplers', [])) if block3 is not None and getattr(block3, 'downsamplers', None) is not None else 0,
            'has_cross_attention': bool(getattr(block3, 'has_cross_attention', False)) if block3 is not None else False,
            'first_resnet_has_conv_shortcut': bool(getattr(block3.resnets[0], 'conv_shortcut', None) is not None) if block3 is not None and getattr(block3, 'resnets', None) else None,
            'note': 'planning preview only; down_blocks.3 is not executed or ported in this stage',
        },
        'effective_weight_plan': {
            'export_conv_weight_key': 'down_blocks_2_downsamplers_0_conv_weight',
            'export_conv_bias_key': 'down_blocks_2_downsamplers_0_conv_bias',
            'use_static_lora_merged_weight': True,
            'generic_runtime_lora_integration': 'PARTIAL/DEFERRED',
        },
        'markers': {
            'TADSR_UNET_DOWNBLOCK2_DOWNSAMPLER_AUDIT': 'PASS',
            'TADSR_UNET_DOWNBLOCK2_DOWNSAMPLER_LORA_AUDIT': 'PASS',
            'TADSR_UNET_DOWNBLOCK2_LOCAL_FORWARD_AUDIT': 'PASS' if local_forward_pass else 'FAIL',
            'TADSR_UNET_DOWNBLOCK2_OUTPUT_STATES_AUDIT': 'PASS' if output_state_pass else 'FAIL',
        },
        'not_in_scope': ['down_blocks.3', 'mid_block', 'up_blocks', 'full UNet forward', 'full TADSR inference'],
    }
    OUT_JSON.write_text(json.dumps(metadata, indent=2, default=str), encoding='utf-8')
    lines = [
        '# TADSR UNet down_blocks.2.downsamplers.0 Audit',
        '',
        f"TADSR_UNET_DOWNBLOCK2_DOWNSAMPLER_AUDIT: {metadata['markers']['TADSR_UNET_DOWNBLOCK2_DOWNSAMPLER_AUDIT']}",
        f"TADSR_UNET_DOWNBLOCK2_DOWNSAMPLER_LORA_AUDIT: {metadata['markers']['TADSR_UNET_DOWNBLOCK2_DOWNSAMPLER_LORA_AUDIT']}",
        f"TADSR_UNET_DOWNBLOCK2_LOCAL_FORWARD_AUDIT: {metadata['markers']['TADSR_UNET_DOWNBLOCK2_LOCAL_FORWARD_AUDIT']}",
        f"TADSR_UNET_DOWNBLOCK2_OUTPUT_STATES_AUDIT: {metadata['markers']['TADSR_UNET_DOWNBLOCK2_OUTPUT_STATES_AUDIT']}",
        '',
        ...
        f"Manual local down_blocks.2 vs official block.forward max abs error: `{metadata['dry_run']['manual_vs_block_forward_max_abs_error']}`",
        f"Output states order: `{metadata['dry_run']['output_states_expected_order']}`",
        '',
        'This audit stops after `down_blocks.2.downsamplers.0`; down_blocks.3, full UNet forward and full TADSR inference remain out of scope.',
    ]
    OUT_TXT.write_text('\n'.join(lines) + '\n', encoding='utf-8')
