...
OUT_JSON = OUT_DIR / 'audit_tadsr_unet_downblock2_attention1.json'
OUT_TXT = OUT_DIR / 'audit_tadsr_unet_downblock2_attention1.txt'
...
def main() -> int:
    ...
    with torch.no_grad():
        ...
        attention1_out = attention1(resnet0_out, encoder_hidden_states=encoder_hidden_states, return_dict=False)[0]
    children = [{'name': n, 'class': f'{type(m).__module__}.{type(m).__name__}'} for n, m in block2.named_children()]
    cfg = attention_config(attention1, 'down_blocks_2_attentions_1', 'unet.down_blocks.2.attentions.1')
    prev_cfg = attention_config(attention_prev, 'down_blocks_1_attentions_1', 'unet.down_blocks.1.attentions.1')
    comparison = {k: {'downblock1_attention1': prev_cfg.get(k), 'downblock2_attention1': cfg.get(k), 'same': prev_cfg.get(k) == cfg.get(k)}
    ...
    audit = {
        'status': 'PASS',
        'scope': 'down_blocks.2.attentions.1 only; full down_blocks.2 and full UNet remain unopened.',
        ...
        'downblock2_overview': {
            ...
            'actual_next_module_after_attention1': 'resnets.1' if resnet1 is not None else None,
        ...
        'effective_weights_strategy': 'Export merged static weights in tools/export_tadsr_unet_downblock2_attention1_oracle.py; no generic Jittor LoRA runtime.',
        'bridge_shapes': {
            ...
            'downblock2_attention1_output': list(attention1_out.shape),
        },
        'bridge_tensor_stats': {
            'downblock2_resnet0_output': stats_tensor(resnet0_out),
            'downblock2_attention1_output': stats_tensor(attention1_out),
        },
        'output_states_preview': {
            'resnets0_output_shape': list(resnet0_out.shape),
            'attentions0_output_shape': list(attention1_out.shape),
        ...
        'next_action': 'Port down_blocks.2.resnets.1 after down_blocks.2.attentions.1 alignment; keep full UNet forward NotImplemented.',
    }
    ...
    lines = [
        'TADSR UNet down_blocks.2.attentions.1 audit',
        ...
        f"output shape: {audit['bridge_shapes']['downblock2_attention1_output']}",
        f"LoRA wrapped modules: {list(lora_modules)}",
        f"next module: {audit['downblock2_overview']['actual_next_module_after_attention1']}",
    ]
    OUT_TXT.write_text('\n'.join(lines) + '\n', encoding='utf-8')
    print(f'TADSR_UNET_DOWNBLOCK2_ATTENTION1_AUDIT: {audit["status"]}')
    if audit['status'] == 'PASS':
        print('TADSR_UNET_DOWNBLOCK2_ATTENTION1_TRANSFORMER_AUDIT: PASS')
        print('TADSR_UNET_DOWNBLOCK2_ATTENTION1_LORA_AUDIT: PASS')
        return 0
    return 1
