...
OUT_JSON = OUT_DIR / 'audit_tadsr_unet_upblock1_resnet2.json'
OUT_TXT = OUT_DIR / 'audit_tadsr_unet_upblock1_resnet2.txt'
...
def main() -> int:
    ...
    if len(up0.resnets) < 3 or not getattr(up0, 'upsamplers', None):
        raise RuntimeError('Official up_blocks.0 must be locally complete before auditing up_blocks.1.resnets.2.')
    ...
    with torch.no_grad():
        ...
        mid, mid_return = extract_midblock_output(mid_raw)
        ...
        attention1_out = attention1(out1, encoder_hidden_states=encoder_hidden_states, return_dict=False)[0]
    ...
    next_module = 'up_blocks.1.attentions.2' if len(getattr(up1, 'attentions', [])) > 2 else 'up_blocks.1.upsamplers.0'
    meta = {
        'status': 'PASS',
        ...
        'upblock1_actual_forward_order_until_target': [
            ...
            'run up_blocks.1.attentions.1 without consuming residuals',
            'pop residual for resnets.2 from the end of the remaining local residual tuple',
            'concat attentions.1 output and popped residual on channel axis',
            'run up_blocks.1.resnets.2',
            f'stop before {next_module}',
        ],
        ...
        'upblock1_resnet2_output_shape': list(out2.shape),
        'concat_axis': 1,
        'temb_shape': list(temb.shape),
        'resnet2_config': resnet_config(resnet2, 'up_blocks_1_resnets_2'),
        'resnet2_lora_info': {
        ...
        'actual_next_module_after_upblock1_resnet2': next_module,
        'markers': {
            ...
            'TADSR_UNET_UPBLOCK1_RESNET2_LORA_AUDIT': 'PASS',
        },
        'scope_boundaries': {
            'audited_this_stage': 'up_blocks.1.resnets.2 residual pop/concat after attentions.1 only',
    ...
    OUT_TXT.write_text('\n'.join([
        '# TADSR UNet up_blocks.1.resnets.2 official audit',
        '',
        *[f'{k}: {v}' for k, v in meta['markers'].items()],
        '',
        f"resnet2 residual source: {meta['upblock1_resnet2_consumed_residual_source']}",
        f"resnet2 residual index: {resnet2_consumed_index}",
        f"hidden input shape: {meta['upblock1_resnet2_hidden_input_shape']}",
        f"concat shape: {meta['upblock1_resnet2_concat_input_shape']}",
        f"output shape: {meta['upblock1_resnet2_output_shape']}",
