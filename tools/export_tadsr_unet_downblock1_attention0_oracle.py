...
ORACLE_DIR = OUT_DIR / 'oracle_tensors_unet_downblock1_attention0'
META_JSON = ORACLE_DIR / 'unet_downblock1_attention0_oracle_metadata.json'
SUMMARY_TXT = ORACLE_DIR / 'oracle_summary.txt'
EFFECTIVE_WEIGHTS = OUT_DIR / 'converted_unet_downblock1_attention0_effective_weights.npz'
PREV_RESNET0_ORACLE = OUT_DIR / 'oracle_tensors_unet_downblock1_resnet0' / 'entry_downblock1_resnet0_output.npy'
...
def main() -> int:
    ...
    h_shape = [1, int(attention0.in_channels), 16, 16]
    ...
    save_tensor(saved, 'synthetic_downblock1_attention0_input', synthetic_hidden)
    save_tensor(saved, 'synthetic_downblock1_attention0_encoder_hidden_states', encoder_hidden_states)
    for k, v in synthetic_manual.items():
        save_tensor(saved, f'synthetic_downblock1_attention0_{k}', v)
    save_tensor(saved, 'synthetic_downblock1_attention0_official_output', synthetic_official)
    ...
    for k, v in entry_manual.items():
        save_tensor(saved, f'entry_downblock1_attention0_{k}', v)
    save_tensor(saved, 'entry_downblock1_attention0_official_output', entry_official)
    arrays = {}
    emeta = {}
    prefix = 'down_blocks_1_attentions_0'
    ...
    previous_resnet0_compare = {'previous_downblock1_resnet0_oracle_tensor_available': False}
    if PREV_RESNET0_ORACLE.exists():
        ...
        previous_resnet0_compare = {
            'previous_downblock1_resnet0_oracle_tensor_available': True,
    ...
    metadata = {
        ...
        'previous_downblock1_resnet0_compare': previous_resnet0_compare,
        ...
        'saved_tensors': saved,
        'markers': {
            'TADSR_UNET_DOWNBLOCK1_ATTENTION0_ORACLE_TENSORS': 'PASS',
            'TADSR_UNET_DOWNBLOCK1_ATTENTION0_EFFECTIVE_WEIGHTS': 'PASS',
        },
        'not_in_scope': ['full down_blocks.1', 'down_blocks.1.resnets.1', 'down_blocks.1.attentions.1', 'down_blocks.1.downsamplers.0', 'full UNet forward', 'full TADSR inference'],
    }
    META_JSON.write_text(json.dumps(metadata, indent=2, default=str), encoding='utf-8')
    lines = [
        '# TADSR UNet down_blocks.1.attentions.0 Oracle Export',
        '',
        'TADSR_UNET_DOWNBLOCK1_ATTENTION0_ORACLE_TENSORS: PASS',
        'TADSR_UNET_DOWNBLOCK1_ATTENTION0_EFFECTIVE_WEIGHTS: PASS',
        '',
        ...
        f"Path B entry->downblock0->downblock1.resnet0->attention0 output shape: `{metadata['path_b_entry_downblock0_downblock1_resnet0_attention0']['output_shape']}`",
        ...
        '',
        'This export stops before down_blocks.1.resnets.1, down_blocks.1.attentions.1, down_blocks.1.downsamplers.0 and full UNet.',
    ]
    SUMMARY_TXT.write_text('\n'.join(lines) + '\n', encoding='utf-8')
    print('TADSR_UNET_DOWNBLOCK1_ATTENTION0_ORACLE_TENSORS: PASS')
    print('TADSR_UNET_DOWNBLOCK1_ATTENTION0_EFFECTIVE_WEIGHTS: PASS')
