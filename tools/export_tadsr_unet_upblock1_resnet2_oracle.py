...
from export_tadsr_unet_upblock1_resnet0_oracle import (
    extract_midblock_output,
    source_names,
)

ORACLE_DIR = OUT_DIR / 'oracle_tensors_unet_upblock1_resnet2'
META_JSON = ORACLE_DIR / 'unet_upblock1_resnet2_oracle_metadata.json'
SUMMARY_TXT = ORACLE_DIR / 'oracle_summary.txt'
EFFECTIVE_WEIGHTS = OUT_DIR / 'converted_unet_upblock1_resnet2_effective_weights.npz'
PREV_UPBLOCK1_ATTENTION1_ORACLE = OUT_DIR / 'oracle_tensors_unet_upblock1_attention1' / 'entry_upblock1_attention1_output.npy'
...
def save_local_resnet_intermediates(saved, prefix, inter, official):
...
def main() -> int:
    ...
    if len(up0.resnets) < 3 or not getattr(up0, 'upsamplers', None):
        raise RuntimeError('Official up_blocks.0 must be locally complete before exporting up_blocks.1.resnets.2.')
    if len(up1.resnets) < 3 or len(getattr(up1, 'attentions', []) or []) < 2:
        raise RuntimeError('Official up_blocks.1 must expose resnets.0/1/2 and attentions.0/1 before this stage.')
    ...
    saved = {}
    for name, tensor in [
        ('synthetic_upblock1_resnet2_hidden_input', synthetic_h),
        ('synthetic_upblock1_resnet2_res_hidden', synthetic_res),
        ('synthetic_upblock1_resnet2_concat_input', synthetic_concat),
        ('synthetic_upblock1_resnet2_temb', synthetic_temb),
    ]:
        save_tensor(saved, name, tensor)
    save_local_resnet_intermediates(saved, 'synthetic_upblock1_resnet2', synthetic_inter, synthetic_official)
    for name, tensor in [
    ...
    for name, tensor in [
        ...
        ('entry_upblock1_resnet2_temb', time_embedding),
    ]:
        save_tensor(saved, name, tensor)
    save_local_resnet_intermediates(saved, 'entry_upblock1_resnet2', entry_inter, entry_official)

    effective = {}
    effective_meta = {}
    prefix = 'up_blocks_1_resnets_2'
    ...
    previous_attention1_compare = {'previous_upblock1_attention1_oracle_tensor_available': False}
    if PREV_UPBLOCK1_ATTENTION1_ORACLE.exists():
        prev = np.load(PREV_UPBLOCK1_ATTENTION1_ORACLE)
        curr = to_np(attention1_out)
        previous_attention1_compare = {
            'previous_upblock1_attention1_oracle_tensor_available': True,
    ...
    metadata = {
        ...
        'upblock1_attention1_consumes_accumulated_residuals': False,
        'upblock1_resnet2_consumed_residual_index_in_accumulated_tuple': resnet2_consumed_index,
        'upblock1_resnet2_consumed_residual_current_remaining_tuple_index': len(remaining_after_resnet1_local) - 1,
        'upblock1_resnet2_consumed_residual_source': sources[resnet2_consumed_index],
        'upblock1_resnet2_consumed_residual_shape': list(res2_hidden.shape),
        'residual_consumption': {
            'target': 'up_blocks.1.resnets.2',
            'consumed_source': sources[resnet2_consumed_index],
            'consumed_index_in_accumulated_tuple': resnet2_consumed_index,
            'current_remaining_tuple_index': len(remaining_after_resnet1_local) - 1,
            'remaining_after_pop_count': len(remaining_after_resnet2_local),
            'remaining_after_pop_shapes': [list(x.shape) for x in remaining_after_resnet2_local],
        },
        'internal_remaining_residual_count_after_resnet2_pop': len(remaining_after_resnet2_local),
        'internal_remaining_residual_shapes_after_resnet2_pop': [list(x.shape) for x in remaining_after_resnet2_local],
        'upblock0_local_output_shape': list(up0_output.shape),
        'upblock1_resnet0_output_shape': list(resnet0_out.shape),
        'upblock1_attention0_output_shape': list(attention0_out.shape),
        'upblock1_resnet1_output_shape': list(resnet1_out.shape),
        'upblock1_attention1_output_shape': list(attention1_out.shape),
        'upblock1_resnet2_hidden_input_shape': list(attention1_out.shape),
        'upblock1_resnet2_concat_input_shape': list(concat2.shape),
        'upblock1_resnet2_output_shape': list(entry_official.shape),
        'concat_axis': 1,
        ...
        'previous_upblock1_attention1_compare': previous_attention1_compare,
        'actual_next_module_after_upblock1_resnet2': 'up_blocks.1.attentions.2' if len(getattr(up1, 'attentions', [])) > 2 else 'up_blocks.1.upsamplers.0',
        'markers': {
            'TADSR_UNET_UPBLOCK1_RESNET2_ORACLE_TENSORS': 'PASS',
            'TADSR_UNET_UPBLOCK1_RESNET2_EFFECTIVE_WEIGHTS': 'PASS',
            'TADSR_UNET_UPBLOCK1_RESNET2_RESIDUAL_CONTRACT_ORACLE': 'PASS',
        },
        'scope_boundaries': {
            'ported_this_stage': 'up_blocks.1.resnets.2 only, including residual consumption after up_blocks.1.attentions.1',
    ...
    SUMMARY_TXT.write_text('\n'.join([
        '# TADSR UNet up_blocks.1.resnets.2 PyTorch oracle',
        '',
        *[f'{k}: {v}' for k, v in metadata['markers'].items()],
        '',
        f"residual source: {metadata['upblock1_resnet2_consumed_residual_source']}",
        f"hidden input shape: {metadata['upblock1_resnet2_hidden_input_shape']}",
        f"concat shape: {metadata['upblock1_resnet2_concat_input_shape']}",
        f"output shape: {metadata['upblock1_resnet2_output_shape']}",
        ...
        f"next unopened module: {metadata['actual_next_module_after_upblock1_resnet2']}",
    ]) + '\n', encoding='utf-8')
