...
OUT_JSON = OUT_DIR / 'audit_tadsr_unet_downblock3_resnet0.json'
OUT_TXT = OUT_DIR / 'audit_tadsr_unet_downblock3_resnet0.txt'
...
def main() -> int:
    ...
    attention0 = block3.attentions[0] if hasattr(block3, 'attentions') and len(block3.attentions) > 0 else None
    resnet1 = block3.resnets[1] if len(getattr(block3, 'resnets', [])) > 1 else None
    downsampler0 = block3.downsamplers[0] if getattr(block3, 'downsamplers', None) is not None and len(block3.downsamplers) > 0 else None
    ...
    accumulated_shapes = [list(conv_in.shape)] + [list(x.shape) for x in block0_states] + [list(x.shape) for x in block1_states] + [list(x.shape) for x in block2_states]
    submodules = {}
    ...
    cfg = resnet_config(resnet0, 'down_blocks_3_resnets_0')
    audit = {
        'status': 'PASS',
        'scope': 'down_blocks.3.resnets.0 only; down_blocks.3 attention/resnet1/downsampler and full UNet forward are intentionally not ported in this stage.',
        ...
        'downblock3_overview': {
            ...
            'downsampler_count': len(getattr(block3, 'downsamplers', [])) if getattr(block3, 'downsamplers', None) is not None else 0,
            'has_downsampler': downsampler0 is not None,
            'has_cross_attention': hasattr(block3, 'attentions') and len(getattr(block3, 'attentions', [])) > 0,
            'current_target_module': 'down_blocks.3.resnets.0',
            'remaining_modules_after_this_stage': ['down_blocks.3.attentions.0' if attention0 is not None else None, 'down_blocks.3.resnets.1' if resnet1 is not None else None, 'down_blocks.3.attentions.1' if hasattr(block3, 'attentions') and len(block3.attentions) > 1 else None, 'down_blocks.3.downsamplers.0' if downsampler0 is not None else None],
            'actual_next_module_after_resnet0': 'down_blocks.3.attentions.0' if attention0 is not None else ('down_blocks.3.resnets.1' if resnet1 is not None else None),
            ...
            'lora_scale_argument_exists': 'scale' in str(inspect.signature(block3.forward)),
        ...
        'tester_support': {
            'existing_UNetResnetBlock3DTester_supported': cfg['time_embedding_norm'] in {'default', 'scale_shift'} and cfg['dropout'] == 0.0,
            ...
            'required_code_changes': [] if cfg['time_embedding_norm'] in {'default', 'scale_shift'} and cfg['dropout'] == 0.0 else ['extend UNetResnetBlock3DTester'],
        },
        'dry_run': {
            ...
            'down_blocks_2_final_hidden_states': stats_tensor(block2_hidden),
            'down_blocks_2_output_state_shapes': [list(x.shape) for x in block2_states],
            'accumulated_down_block_res_sample_shapes': accumulated_shapes,
            'down_blocks_3_resnets_0_input': stats_tensor(block2_hidden),
            'down_blocks_3_resnets_0_output': stats_tensor(resnet0_out),
        ...
        'markers': {
            'TADSR_UNET_DOWNBLOCK3_AUDIT': 'PASS',
            'TADSR_UNET_DOWNBLOCK3_RESNET0_AUDIT': 'PASS',
            'TADSR_UNET_DOWNBLOCK3_RESNET0_LORA_AUDIT': 'PASS',
        },
        'not_in_scope': ['down_blocks.3.attentions.0', 'down_blocks.3.resnets.1', 'down_blocks.3.attentions.1', 'down_blocks.3.downsamplers.0', 'mid_block', 'up_blocks', 'full UNet forward', 'full TADSR inference', 'generic LoRA runtime'],
    }
    OUT_JSON.write_text(json.dumps(audit, indent=2, default=str), encoding='utf-8')
    lines = [
        '# TADSR UNet down_blocks.3.resnets.0 Audit',
        '',
        'TADSR_UNET_DOWNBLOCK3_AUDIT: PASS',
        'TADSR_UNET_DOWNBLOCK3_RESNET0_AUDIT: PASS',
        'TADSR_UNET_DOWNBLOCK3_RESNET0_LORA_AUDIT: PASS',
        '',
        f"Block class: `{audit['downblock3_overview']['class']}`",
        f"ResNet0 channels: `{cfg['in_channels']} -> {cfg['out_channels']}`, shortcut: `{cfg['has_shortcut']}`",
        f"Input shape: `{audit['dry_run']['down_blocks_3_resnets_0_input']['shape']}`, output shape: `{audit['dry_run']['down_blocks_3_resnets_0_output']['shape']}`",
        f"Next module preview: `{audit['downblock3_overview']['actual_next_module_after_resnet0']}`",
        '',
        'This audit stops after `down_blocks.3.resnets.0`; attention0, full down_blocks.3, full UNet forward and full TADSR inference remain out of scope.',
    ]
    OUT_TXT.write_text('\n'.join(lines) + '\n', encoding='utf-8')
