...
OUT_JSON = OUT_DIR / 'audit_tadsr_unet_upblock1_attention0.json'
OUT_TXT = OUT_DIR / 'audit_tadsr_unet_upblock1_attention0.txt'
...
def main() -> int:
    ...
    if len(up1.resnets) < 2 or len(getattr(up1, 'attentions', []) or []) < 1:
        raise RuntimeError('Official up_blocks.1 must expose attentions.0 and following resnets.1 for this stage.')
    ...
    cfg = attention_config(attention0, 'up_blocks_1_attentions_0', 'unet.up_blocks.1.attentions.0')
    ...
    result = {
        'status': 'PASS',
        'scope': 'up_blocks.1.attentions.0 audit only; up_blocks.1.resnets.1 and later modules remain intentionally unopened.',
        ...
        'upblock1_overview': {
            ...
            'known_completed_modules_before_this_stage': ['up_blocks.1.resnets.0'],
            'target_module': 'up_blocks.1.attentions.0',
            'actual_next_module_after_attention0': 'up_blocks.1.resnets.1',
            'remaining_modules_after_this_stage': ['up_blocks.1.resnets.1', 'up_blocks.1.attentions.1', 'up_blocks.1.resnets.2', 'up_blocks.1.attentions.2', 'up_blocks.1.upsamplers.0'],
            'expected_operation_order_until_target': [
                ...
                'stop before up_blocks.1.resnets.1',
            ],
        },
        ...
        'entry_context_shapes': {
            ...
            'upblock1_resnet0_residual_source': sources[consumed_index],
            ...
            'upblock1_attention0_output': list(att0.shape),
        ...
        'existing_tester_compatibility': {
            ...
            'reason': 'Same Transformer2DModel + BasicTransformerBlock pattern; selected with prefix=up_blocks_1_attentions_0.',
        },
        'lora_modules': lora_modules,
        'downstream_resnet1_preview': {
            'exists': len(up1.resnets) > 1,
            'class': f'{type(up1.resnets[1]).__module__}.{type(up1.resnets[1]).__name__}',
        ...
        'markers': {
            ...
            'TADSR_UNET_UPBLOCK1_ATTENTION0_LORA_AUDIT': 'PASS',
        },
        'scope_boundaries': {
            'ported_this_stage': 'up_blocks.1.attentions.0 only',
            'not_ported_this_stage': ['up_blocks.1.resnets.1', 'up_blocks.1.attentions.1', 'up_blocks.1.resnets.2', 'up_blocks.1.attentions.2', 'up_blocks.1.upsamplers.0', 'up_blocks.2', 'up_blocks.3', 'full UNet forward', 'generic runtime LoRA', 'full TADSR inference'],
        },
    }
    OUT_JSON.write_text(json.dumps(result, indent=2), encoding='utf-8')
    OUT_TXT.write_text('\n'.join([
        '# TADSR UNet up_blocks.1.attentions.0 official audit',
        '',
        *[f'{k}: {v}' for k, v in result['markers'].items()],
        ...
        f"input shape: {list(resnet0_out.shape)}",
        f"output shape: {list(att0.shape)}",
        'next module: up_blocks.1.resnets.1',
