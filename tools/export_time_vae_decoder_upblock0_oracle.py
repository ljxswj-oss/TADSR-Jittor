...
PREV_MIDBLOCK_ORACLE = BASE_OUT / 'oracle_tensors_decoder_midblock'
...
def run_upblock0_parts(block, x, temb):
    hidden = x
    res_outputs = []
...
def export_path(out, arrays, stats, prefix, source_label, upblock0, hidden, temb):
    res_outputs, pre_upsampler, upsampler_outputs, direct = run_upblock0_parts(upblock0, hidden, temb)
    for i, value in enumerate(res_outputs):
        save(out, arrays, f'{prefix}_decoder_upblock0_resnet{i}_output', to_np(value), f'decoder.up_blocks.0.resnets.{i}({source_label})', stats)
    save(out, arrays, f'{prefix}_decoder_upblock0_pre_upsampler_output', to_np(pre_upsampler), f'decoder.up_blocks.0.pre_upsampler({source_label})', stats)
    for i, value in enumerate(upsampler_outputs):
        save(out, arrays, f'{prefix}_decoder_upblock0_upsampler{i}_output', to_np(value), f'decoder.up_blocks.0.upsamplers.{i}({source_label})', stats)
    save(out, arrays, f'{prefix}_decoder_upblock0_output', to_np(direct), f'decoder.up_blocks.0({source_label})', stats)
    return direct
...
def main() -> int:
    ...
    out = BASE_OUT / 'oracle_tensors_decoder_upblock0'
    ...
    with torch.no_grad():
        ...
        # Path A: isolated synthetic hidden -> decoder.up_blocks.0.
        ...
        save(out, arrays, 'synthetic_decoder_upblock0_input', synthetic_hidden_np, 'path_a.synthetic_linspace_hidden', stats)
        export_path(out, arrays, stats, 'synthetic', 'synthetic', upblock0, synthetic_hidden, decoder_temb)

        # Path B: synthetic latent -> post_quant_conv -> decoder.conv_in -> decoder.mid_block -> decoder.up_blocks.0.
        prev_z = PREV_MIDBLOCK_ORACLE / 'entry_synthetic_latent_z.npy'
        if prev_z.exists():
        ...
        export_path(out, arrays, stats, 'entry', 'entry', upblock0, entry_mid, decoder_temb)

        # Path C: real encoder deterministic path -> decoder.up_blocks.0.
        prev_img = PREV_MIDBLOCK_ORACLE / 'real_input_image.npy'
        if prev_img.exists():
        ...
        export_path(out, arrays, stats, 'real', 'path_c', upblock0, real_mid, decoder_temb)

    prev_diff = None
    prev_real_mid = PREV_MIDBLOCK_ORACLE / 'real_decoder_midblock_output.npy'
    if prev_real_mid.exists():
        prev = np.load(prev_real_mid).astype(np.float32)
        prev_diff = float(np.max(np.abs(prev - arrays['real_decoder_midblock_output'])))

    np.savez_compressed(out / 'time_vae_decoder_upblock0_inputs.npz', **{
        k: arrays[k] for k in [
            'synthetic_decoder_upblock0_input',
            'entry_synthetic_latent_z',
            'real_input_image',
            'real_encoder_temb',
            'real_moments',
            'real_decoder_midblock_output',
        ]
    })
    np.savez_compressed(out / 'time_vae_decoder_upblock0_outputs.npz', **{
    ...
    meta = {
        ...
        'decoder_upblock0_class': f'{upblock0.__class__.__module__}.{upblock0.__class__.__name__}',
        'uses_sampling': False,
        'decoder_side_temb_is_none': True,
        'upblock0_resnet_count': len(upblock0.resnets),
        'upblock0_upsampler_count': len(upblock0.upsamplers or []),
        'upblock0_has_upsampler': bool(upblock0.upsamplers),
        'upblock0_module_order': [f'resnets.{i}' for i in range(len(upblock0.resnets))] + [f'upsamplers.{i}' for i in range(len(upblock0.upsamplers or []))],
        'upblock0_input_shape': list(arrays['real_decoder_midblock_output'].shape),
        'upblock0_output_shape': list(arrays['real_decoder_upblock0_output'].shape),
        ...
        'max_abs_diff_between_real_decoder_midblock_output_and_previous_decoder_midblock_oracle': prev_diff,
        'tensor_stats': stats,
    }
    (out / 'decoder_upblock0_oracle_metadata.json').write_text(json.dumps(meta, indent=2), encoding='utf-8')
    lines = [
        '# TimeAware VAE Decoder UpBlock0 Oracle Export',
        '',
        f"Status: **{meta['status']}**",
        '',
        f"Decoder upblock0 class: `{meta['decoder_upblock0_class']}`",
        f"Module order: `{meta['upblock0_module_order']}`",
        f"ResNet count: `{meta['upblock0_resnet_count']}`",
        f"Upsampler count: `{meta['upblock0_upsampler_count']}`",
        f"Decoder-side temb is None: `{meta['decoder_side_temb_is_none']}`",
        f"Previous decoder-midblock real output diff: `{prev_diff}`",
        '',
        '## Paths',
        '',
        '- Path A: synthetic hidden -> decoder.up_blocks.0.',
        '- Path B: synthetic latent -> post_quant_conv -> decoder.conv_in -> decoder.mid_block -> decoder.up_blocks.0.',
        '- Path C: synthetic image -> encoder -> quant_conv -> posterior mode -> post_quant_conv -> decoder.conv_in -> decoder.mid_block -> decoder.up_blocks.0.',
    ]
