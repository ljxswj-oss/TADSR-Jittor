...
ORACLE_DIR = OUT_DIR / 'oracle_tensors_unet_upblock2_resnet2'
META_JSON = ORACLE_DIR / 'unet_upblock2_resnet2_oracle_metadata.json'
SUMMARY_TXT = ORACLE_DIR / 'oracle_summary.txt'
EFFECTIVE_WEIGHTS = OUT_DIR / 'converted_unet_upblock2_resnet2_effective_weights.npz'
PREV_UPBLOCK2_ATTENTION1_ORACLE = OUT_DIR / 'oracle_tensors_unet_upblock2_attention1' / 'entry_upblock2_attention1_output.npy'
...
def main() -> int:
    ...
    saved = {}
    for name, tensor in [
        ('synthetic_upblock2_resnet2_hidden_input', synthetic_hidden),
        ('synthetic_upblock2_resnet2_res_hidden', synthetic_res),
        ('synthetic_upblock2_resnet2_concat_input', synthetic_concat),
        ('synthetic_upblock2_resnet2_temb', synthetic_temb),
    ]:
        save_tensor(saved, name, tensor)
    save_resnet_intermediates(saved, 'synthetic_upblock2_resnet2', synthetic_inter, synthetic_official)
    ...
    for name, tensor in [
        ...
        save_tensor(saved, name, tensor)
    save_resnet_intermediates(saved, 'entry_upblock2_resnet2', entry_inter, entry_official)

    effective = {}
    effective_meta = {}
    prefix = 'up_blocks_2_resnets_2'
    ...
    previous_attention1_compare = {'previous_upblock2_attention1_oracle_tensor_available': False}
    if PREV_UPBLOCK2_ATTENTION1_ORACLE.exists():
        prev = np.load(PREV_UPBLOCK2_ATTENTION1_ORACLE)
        curr = to_np(attention1_out)
        previous_attention1_compare = {
            'previous_upblock2_attention1_oracle_tensor_available': True,
    ...
    metadata = {
        ...
        'upblock2_resnet2_residual_input_shape': list(res2_hidden.shape),
        'upblock2_resnet2_concat_input_shape': list(concat2.shape),
        'upblock2_resnet2_output_shape': list(entry_official.shape),
        ...
        'previous_upblock2_attention1_compare': previous_attention1_compare,
        ...
        'markers': {
            'TADSR_UNET_UPBLOCK2_RESNET2_ORACLE_TENSORS': 'PASS',
            'TADSR_UNET_UPBLOCK2_RESNET2_EFFECTIVE_WEIGHTS': 'PASS',
            'TADSR_UNET_UPBLOCK2_RESNET2_RESIDUAL_CONTRACT_ORACLE': 'PASS',
        },
    }
    META_JSON.write_text(json.dumps(metadata, indent=2), encoding='utf-8')
    SUMMARY_TXT.write_text('\n'.join([
        '# TADSR UNet up_blocks.2.resnets.2 PyTorch oracle',
        '',
        *[f'{k}: {v}' for k, v in metadata['markers'].items()],
        '',
        f"residual source: {metadata['upblock2_resnet2_consumed_residual_source']}",
        f"hidden input shape: {metadata['upblock2_resnet2_hidden_input_shape']}",
        f"residual shape: {metadata['upblock2_resnet2_residual_input_shape']}",
        f"concat shape: {metadata['upblock2_resnet2_concat_input_shape']}",
        f"output shape: {metadata['upblock2_resnet2_output_shape']}",
