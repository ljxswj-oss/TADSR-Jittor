...
OUT_JSON = OUT_DIR / 'audit_tadsr_unet_downblock1_attention0.json'
OUT_TXT = OUT_DIR / 'audit_tadsr_unet_downblock1_attention0.txt'
...
def main() -> int:
    ...
    attention_prev = block0.attentions[1]
    ...
    cfg = attention_config(attention0, 'down_blocks_1_attentions_0', 'unet.down_blocks.1.attentions.0')
    prev_cfg = attention_config(attention_prev, 'down_blocks_0_attentions_1', 'unet.down_blocks.0.attentions.1')
    comparison = {k: {'downblock0_attention1': prev_cfg.get(k), 'downblock1_attention0': cfg.get(k), 'same': prev_cfg.get(k) == cfg.get(k)}
    ...
    audit = {
        'status': 'PASS',
        'scope': 'down_blocks.1.attentions.0 only; the rest of down_blocks.1 and full UNet remain unopened.',
        ...
        'compare_with_downblock0_attention1': comparison,
        ...
        'effective_weights_strategy': 'Export merged static weights in tools/export_tadsr_unet_downblock1_attention0_oracle.py; no generic Jittor LoRA runtime.',
        'bridge_shapes': {
        ...
        'not_in_scope': ['down_blocks.1.resnets.1', 'down_blocks.1.attentions.1', 'down_blocks.1.downsamplers.0', 'mid_block', 'up_blocks', 'full UNet forward', 'full TADSR inference', 'generic Jittor LoRA runtime'],
        'next_action': 'Port down_blocks.1.resnets.1 after down_blocks.1.attentions.0 alignment; keep full UNet forward NotImplemented.',
    }
    ...
    lines = [
        'TADSR UNet down_blocks.1.attentions.0 audit',
        ...
        f"input shape: {audit['bridge_shapes']['downblock1_resnet0_output']}",
        f"output shape: {audit['bridge_shapes']['downblock1_attention0_output']}",
        f"LoRA wrapped modules: {list(lora_modules)}",
        f"next module: {audit['downblock1_overview']['actual_next_module_after_attention0']}",
    ]
    OUT_TXT.write_text('\n'.join(lines) + '\n', encoding='utf-8')
    print(f'TADSR_UNET_DOWNBLOCK1_ATTENTION0_AUDIT: {audit["status"]}')
    if audit['status'] == 'PASS':
        print('TADSR_UNET_DOWNBLOCK1_ATTENTION0_TRANSFORMER_AUDIT: PASS')
        print('TADSR_UNET_DOWNBLOCK1_ATTENTION0_LORA_AUDIT: PASS')
        return 0
    return 1
