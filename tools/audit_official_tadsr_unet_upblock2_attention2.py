...
OUT_JSON = OUT_DIR / 'audit_tadsr_unet_upblock2_attention2.json'
OUT_TXT = OUT_DIR / 'audit_tadsr_unet_upblock2_attention2.txt'
...
def main() -> int:
    ...
    cfg = attention_config(attention2, 'up_blocks_2_attentions_2', 'unet.up_blocks.2.attentions.2')
    ...
    result = {
        'status': 'PASS',
        'scope': 'up_blocks.2.attentions.2 audit only; up_blocks.2.upsamplers.0 and later modules remain intentionally unopened.',
        ...
        'upblock2_overview': {
            ...
            'target_module': 'up_blocks.2.attentions.2',
            ...
            'expected_operation_order_until_target': [
                ...
                'up_blocks.2.resnets.2 consumes the next residual',
                'run up_blocks.2.attentions.2 with encoder_hidden_states',
                'stop before up_blocks.2.upsamplers.0',
            ],
        },
        ...
        'entry_context_shapes': {
            ...
            'upblock2_resnet2_output': list(resnet2_out.shape),
            'upblock2_attention2_input': list(resnet2_out.shape),
            'upblock2_attention2_output': list(attention2_out.shape),
        ...
        'existing_tester_compatibility': {
            ...
            'reason': 'Same Transformer2DModel + BasicTransformerBlock pattern; selected with prefix=up_blocks_2_attentions_2.',
        },
        'lora_modules': lora_modules,
        'residual_contract': {
            'status': 'PASS',
            'attention2_consumes_accumulated_residuals': False,
            'remaining_residual_count_unchanged': len(remaining_after_resnet2_local),
        },
        'lora_audit': {
            'status': 'PASS',
        ...
        'markers': {
            'TADSR_UNET_UPBLOCK2_ATTENTION2_AUDIT': 'PASS',
            'TADSR_UNET_UPBLOCK2_ATTENTION2_RESIDUAL_CONTRACT_AUDIT': 'PASS',
            'TADSR_UNET_UPBLOCK2_ATTENTION2_LORA_AUDIT': 'PASS',
        },
    }
    OUT_JSON.write_text(json.dumps(result, indent=2), encoding='utf-8')
    OUT_TXT.write_text('\n'.join([
        '# TADSR UNet up_blocks.2.attentions.2 official audit',
        '',
        *[f'{k}: {v}' for k, v in result['markers'].items()],
        '',
        f"target: {result['upblock2_overview']['target_module']}",
        f"next unopened module: {result['upblock2_overview']['actual_next_module_after_attention2']}",
        f"attention input shape: {result['entry_context_shapes']['upblock2_attention2_input']}",
        f"attention output shape: {result['entry_context_shapes']['upblock2_attention2_output']}",
    ]) + '\n', encoding='utf-8')
    for k, v in result['markers'].items():
        print(f'{k}: {v}')
    print(json.dumps({'status': 'PASS', 'audit_json': str(OUT_JSON), 'next': next_module}, indent=2))
    return 0
