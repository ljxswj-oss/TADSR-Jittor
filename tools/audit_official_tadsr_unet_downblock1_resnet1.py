...
OUT_JSON = OUT_DIR / 'audit_tadsr_unet_downblock1_resnet1.json'
OUT_TXT = OUT_DIR / 'audit_tadsr_unet_downblock1_resnet1.txt'
...
def main() -> int:
    ...
    attention1 = block1.attentions[1] if hasattr(block1, 'attentions') and len(block1.attentions) > 1 else None
    ...
    if resnet1 is None:
        audit = {'status': 'BLOCKED_NO_RESNET1', 'reason': 'unet.down_blocks[1].resnets[1] does not exist'}
        OUT_JSON.write_text(json.dumps(audit, indent=2), encoding='utf-8')
        print('TADSR_UNET_DOWNBLOCK1_RESNET1_AUDIT: BLOCKED_NO_RESNET1')
        return 1
    ...
    audit = {
        'status': 'PASS',
        'scope': 'down_blocks.1.resnets.1 only; down_blocks.1.attentions.1/downsampler and full UNet forward are intentionally not ported.',
        ...
        'downblock1_overview': {
            ...
            'remaining_modules_after_this_stage': ['attentions.1' if attention1 is not None else None, 'downsamplers.0' if downsampler0 is not None else None],
            'actual_next_module_after_resnet1': 'attentions.1' if attention1 is not None else ('downsamplers.0' if downsampler0 is not None else None),
            ...
            'has_peft_wrappers': any(v.get('is_lora_wrapped') for v in submodules.values() if isinstance(v, dict)),
        },
        'resnet1_config': resnet_config(resnet1, 'down_blocks_1_resnets_1'),
        'compare_with_resnet0': {
            'resnet0_config': resnet_config(resnet0, 'down_blocks_1_resnets_0'),
            'same_in_channels': getattr(resnet0, 'in_channels', None) == getattr(resnet1, 'in_channels', None),
            'same_out_channels': getattr(resnet0, 'out_channels', None) == getattr(resnet1, 'out_channels', None),
            'resnet1_channel_change': getattr(resnet1, 'in_channels', None) != getattr(resnet1, 'out_channels', None),
            'resnet1_has_shortcut': getattr(resnet1, 'conv_shortcut', None) is not None,
            'existing_tester_support': 'UNetResnetBlock2DTester supports this via prefix down_blocks_1_resnets_1 and resnet1_config metadata.',
        },
        'submodules': submodules,
        'attention1_preview': attention_preview(attention1),
        'bridge_shapes': {
            ...
            'downblock1_resnet1_input': list(attention0_out.shape),
            'downblock1_resnet1_output': list(resnet1_out.shape),
        },
        'bridge_tensor_stats': {
            'downblock1_attention0_output': stats_tensor(attention0_out),
            'downblock1_resnet1_output': stats_tensor(resnet1_out),
        },
        'output_states_preview': {
            'resnet1_output_immediately_added_to_output_states': False,
            'likely_residual_after_attention1': True,
            'note': 'Diffusers CrossAttnDownBlock2D appends output_states after attention pair output; this stage stops before attentions.1.',
            'future_up_blocks_skip_residuals_complete': False,
        },
        'effective_weights_required': True,
        'effective_weights_strategy': 'Export merged static weights in tools/export_tadsr_unet_downblock1_resnet1_oracle.py; no generic Jittor LoRA runtime.',
        'not_in_scope': ['down_blocks.1.attentions.1', 'down_blocks.1.downsamplers.0', 'down_blocks.2', 'mid_block', 'up_blocks', 'full UNet forward', 'full TADSR inference', 'generic Jittor LoRA runtime'],
        'next_action': 'Port down_blocks.1.attentions.1 after down_blocks.1.resnets.1 alignment; keep full UNet forward NotImplemented.',
    }
    OUT_JSON.write_text(json.dumps(audit, indent=2, default=str), encoding='utf-8')
    lines = [
        'TADSR UNet down_blocks.1.resnets.1 audit',
        f"status: {audit['status']}",
        f"resnet class: {audit['resnet1_config']['class']}",
        f"in_channels: {audit['resnet1_config']['in_channels']}",
        f"out_channels: {audit['resnet1_config']['out_channels']}",
        f"temb_channels: {audit['resnet1_config']['temb_channels']}",
        f"has_shortcut: {audit['resnet1_config']['has_shortcut']}",
        f"input shape: {audit['bridge_shapes']['downblock1_resnet1_input']}",
        f"output shape: {audit['bridge_shapes']['downblock1_resnet1_output']}",
        f"next module: {audit['downblock1_overview']['actual_next_module_after_resnet1']}",
    ]
    OUT_TXT.write_text('\n'.join(lines) + '\n', encoding='utf-8')
    print('TADSR_UNET_DOWNBLOCK1_RESNET1_AUDIT: PASS')
    print('TADSR_UNET_DOWNBLOCK1_RESNET1_LORA_AUDIT: PASS')
    return 0
