...
OUT_JSON = OUT_DIR / 'audit_tadsr_unet_upblock3_resnet2.json'
OUT_TXT = OUT_DIR / 'audit_tadsr_unet_upblock3_resnet2.txt'
...
def attention_preview(att, expected_input_shape=None, encoder_hidden_states_shape=None):
    ...
    out = {
        ...
        'expected_input_shape_from_resnet2': expected_input_shape,
...
def main() -> int:
    ...
    attention2 = up3.attentions[2] if len(getattr(up3, 'attentions', []) or []) > 2 else None
    ...
    residual_status = 'PASS' if (
        ...
        and len(remaining_after_resnet1_local) >= 1
        and concat2.shape[1] == attention1_out.shape[1] + res2_hidden.shape[1]
        and len(remaining_after_resnet2_local) == 0
    ) else 'FAIL'
    cfg = resnet_config(resnet2, 'up_blocks_3_resnets_2')
    submodules = {}
    for name in ['norm1', 'conv1', 'time_emb_proj', 'norm2', 'conv2', 'conv_shortcut']:
        module = getattr(resnet2, name, None)
        if module is not None:
            submodules[name] = module_lora_info(module)
    next_name = next_module_name(up3)
    markers = {
        'TADSR_UNET_UPBLOCK3_RESNET2_AUDIT': 'PASS',
        'TADSR_UNET_UPBLOCK3_RESNET2_RESIDUAL_CONTRACT_AUDIT': residual_status,
        'TADSR_UNET_UPBLOCK3_RESNET2_LORA_AUDIT': 'PASS',
    }
    result = {
        ...
        'upblock3_topology': {
            ...
            'target': 'up_blocks.3.resnets.2',
            'actual_next_module_after_resnet2': next_name,
        ...
        'resnet2_config': cfg,
        'resnet2_submodule_lora_info': submodules,
        'residual_consumption': {
            ...
            'remaining_local_residual_count_before_resnet2': len(remaining_after_resnet1_local),
            'remaining_local_residual_shapes_before_resnet2': [list(x.shape) for x in remaining_after_resnet1_local],
            ...
            'upblock3_resnet2_consumed_residual_local_tuple_index_before_pop': len(remaining_after_resnet1_local) - 1,
            'upblock3_resnet2_consumed_source': sources[res2_global_index],
            ...
            'upblock3_resnet2_concat_axis': 1,
            'upblock3_resnet2_concat_shape': list(concat2.shape),
            'remaining_local_residual_count_after_resnet2': len(remaining_after_resnet2_local),
            'remaining_local_residual_shapes_after_resnet2': [list(x.shape) for x in remaining_after_resnet2_local],
        },
        'input_output_dryrun': {
            ...
            'upblock3_resnet2_residual_shape': list(res2_hidden.shape),
            'upblock3_resnet2_concat_shape': list(concat2.shape),
            'upblock3_resnet2_output_shape': list(resnet2_out.shape),
            'upblock3_resnet2_output_stats': stats_tensor(resnet2_out),
            'upblock3_resnet2_output_numpy_stats': {
                'shape': list(to_np(resnet2_out).shape),
                'dtype': str(to_np(resnet2_out).dtype),
            },
        },
        ...
        'next_module_preview': {
            'name': next_name,
            'attention2': attention_preview(attention2, list(resnet2_out.shape), encoder_shape),
            'not_ported_this_stage': True,
        },
    ...
    OUT_TXT.write_text('\n'.join([
        '# TADSR UNet up_blocks.3.resnets.2 audit',
        '',
        *[f'{k}: {v}' for k, v in markers.items()],
        ...
        f"resnet2 consumed source: {result['residual_consumption']['upblock3_resnet2_consumed_source']}",
        f"residual index: {res2_global_index}",
        f"hidden input shape: {result['residual_consumption']['upblock3_resnet2_hidden_input_shape']}",
        f"residual shape: {result['residual_consumption']['upblock3_resnet2_consumed_residual_shape']}",
        f"concat shape: {result['residual_consumption']['upblock3_resnet2_concat_shape']}",
        f"output shape: {result['input_output_dryrun']['upblock3_resnet2_output_shape']}",
        f"actual next module: {next_name}",
        'scope: up_blocks.3 topology and resnets.2 only; attention2/full up_blocks/full UNet/full inference were not executed.',
    ]) + '\n', encoding='utf-8')
