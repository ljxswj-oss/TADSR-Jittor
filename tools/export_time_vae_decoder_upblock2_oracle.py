...
PREV_UPBLOCK1_ORACLE = BASE_OUT / 'oracle_tensors_decoder_upblock1'
AUDIT_PATH = BASE_OUT / 'audit_time_vae_decoder_upblock2.json'
...
def export_upblock2_path(out, arrays, stats, prefix, source_label, upblock2, hidden, temb):
    res_outputs, pre_upsampler, upsampler_outputs, direct = run_upblock_parts(upblock2, hidden, temb)
    for i, value in enumerate(res_outputs):
        save(out, arrays, f'{prefix}_decoder_upblock2_resnet{i}_output', to_np(value), f'decoder.up_blocks.2.resnets.{i}({source_label})', stats)
    save(out, arrays, f'{prefix}_decoder_upblock2_pre_upsampler_output', to_np(pre_upsampler), f'decoder.up_blocks.2.pre_upsampler({source_label})', stats)
    for i, value in enumerate(upsampler_outputs):
        save(out, arrays, f'{prefix}_decoder_upblock2_upsampler{i}_output', to_np(value), f'decoder.up_blocks.2.upsamplers.{i}({source_label})', stats)
    save(out, arrays, f'{prefix}_decoder_upblock2_output', to_np(direct), f'decoder.up_blocks.2({source_label})', stats)
    return direct
...
def main() -> int:
    ...
    out = BASE_OUT / 'oracle_tensors_decoder_upblock2'
    ...
    with torch.no_grad():
        ...
        # Path A: isolated synthetic hidden -> decoder.up_blocks.2.
        ...
        save(out, arrays, 'synthetic_decoder_upblock2_input', synthetic_hidden_np, 'path_a.synthetic_linspace_hidden', stats)
        export_upblock2_path(out, arrays, stats, 'synthetic', 'synthetic', upblock2, synthetic_hidden, decoder_temb)

        # Path B: synthetic latent -> post_quant_conv -> decoder.conv_in -> mid -> upblock0 -> upblock1 -> upblock2.
        prev_z = PREV_UPBLOCK1_ORACLE / 'entry_synthetic_latent_z.npy'
        if prev_z.exists():
        ...
        export_upblock2_path(out, arrays, stats, 'entry', 'entry', upblock2, entry_up1, decoder_temb)

        # Path C: real encoder deterministic path -> decoder.up_blocks.2.
        prev_img = PREV_UPBLOCK1_ORACLE / 'real_input_image.npy'
        if prev_img.exists():
        ...
        export_upblock2_path(out, arrays, stats, 'real', 'path_c', upblock2, real_up1, decoder_temb)

    prev_diff = None
    prev_real_up1 = PREV_UPBLOCK1_ORACLE / 'real_decoder_upblock1_output.npy'
    if prev_real_up1.exists():
        prev = np.load(prev_real_up1).astype(np.float32)
        prev_diff = float(np.max(np.abs(prev - arrays['real_decoder_upblock1_output'])))

    np.savez_compressed(out / 'time_vae_decoder_upblock2_inputs.npz', **{
        k: arrays[k] for k in [
            'synthetic_decoder_upblock2_input',
            'entry_synthetic_latent_z',
            'entry_decoder_upblock1_output',
            'real_input_image',
            'real_encoder_temb',
            'real_moments',
            'real_decoder_upblock1_output',
        ]
    })
    np.savez_compressed(out / 'time_vae_decoder_upblock2_outputs.npz', **{
        k: v for k, v in arrays.items() if k.endswith('_output') or k in {'real_latent_dist_mode'}
    })

    resnet_shortcuts = []
    for i, resnet in enumerate(upblock2.resnets):
    ...
    meta = {
        ...
        'decoder_upblock2_class': f'{upblock2.__class__.__module__}.{upblock2.__class__.__name__}',
        'uses_sampling': False,
        'decoder_side_temb_is_none': True,
        'upblock2_resnet_count': len(upblock2.resnets),
        'upblock2_upsampler_count': len(upblock2.upsamplers or []),
        'upblock2_has_upsampler': bool(upblock2.upsamplers),
        'upblock2_attention_count': len(getattr(upblock2, 'attentions', []) or []),
        'upblock2_module_order': [f'resnets.{i}' for i in range(len(upblock2.resnets))] + [f'upsamplers.{i}' for i in range(len(upblock2.upsamplers or []))],
        'upblock2_input_shape': list(arrays['real_decoder_upblock1_output'].shape),
        'upblock2_output_shape': list(arrays['real_decoder_upblock2_output'].shape),
        'upblock2_channel_change': arrays['real_decoder_upblock1_output'].shape[1] != arrays['real_decoder_upblock2_output'].shape[1],
        'upblock2_shortcut_modules': resnet_shortcuts,
        ...
        'max_abs_diff_between_real_decoder_upblock1_output_and_previous_decoder_upblock1_oracle': prev_diff,
        'tensor_stats': stats,
    }
    (out / 'decoder_upblock2_oracle_metadata.json').write_text(json.dumps(meta, indent=2), encoding='utf-8')
    lines = [
        '# TimeAware VAE Decoder UpBlock2 Oracle Export',
        '',
        f"Status: **{meta['status']}**",
        '',
        f"Decoder upblock2 class: `{meta['decoder_upblock2_class']}`",
        f"Module order: `{meta['upblock2_module_order']}`",
        f"ResNet count: `{meta['upblock2_resnet_count']}`",
        f"Upsampler count: `{meta['upblock2_upsampler_count']}`",
        f"Channel change: `{meta['upblock2_channel_change']}`",
        f"Shortcuts: `{meta['upblock2_shortcut_modules']}`",
        f"Decoder-side temb is None: `{meta['decoder_side_temb_is_none']}`",
        f"Previous decoder-upblock1 real output diff: `{prev_diff}`",
        '',
        '## Paths',
        '',
        '- Path A: synthetic hidden -> decoder.up_blocks.2.',
        '- Path B: synthetic latent -> post_quant_conv -> decoder.conv_in -> decoder.mid_block -> decoder.up_blocks.0 -> decoder.up_blocks.1 -> decoder.up_blocks.2.',
        '- Path C: synthetic image -> encoder -> quant_conv -> posterior mode -> post_quant_conv -> decoder.conv_in -> decoder.mid_block -> decoder.up_blocks.0 -> decoder.up_blocks.1 -> decoder.up_blocks.2.',
    ]
