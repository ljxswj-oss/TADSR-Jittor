...
ORACLE_DIR = OUT_DIR / 'oracle_tensors_unet_downblock1_attention1'
META_JSON = ORACLE_DIR / 'unet_downblock1_attention1_oracle_metadata.json'
SUMMARY_TXT = ORACLE_DIR / 'oracle_summary.txt'
EFFECTIVE_WEIGHTS = OUT_DIR / 'converted_unet_downblock1_attention1_effective_weights.npz'
PREV_RESNET1_ORACLE = OUT_DIR / 'oracle_tensors_unet_downblock1_resnet1' / 'entry_downblock1_resnet1_output.npy'
...
def main() -> int:
    ...
    h_shape = [1, int(attention1.in_channels), 16, 16]
    ...
    save_tensor(saved, 'synthetic_downblock1_attention1_input', synthetic_hidden)
    save_tensor(saved, 'synthetic_downblock1_attention1_encoder_hidden_states', encoder_hidden_states)
    for k, v in synthetic_manual.items():
        save_tensor(saved, f'synthetic_downblock1_attention1_{k}', v)
    save_tensor(saved, 'synthetic_downblock1_attention1_official_output', synthetic_official)
    ...
    for k, v in entry_manual.items():
        save_tensor(saved, f'entry_downblock1_attention1_{k}', v)
    save_tensor(saved, 'entry_downblock1_attention1_official_output', entry_official)
    arrays = {}
    emeta = {}
    prefix = 'down_blocks_1_attentions_1'
    ...
    previous_resnet1_compare = {'previous_downblock1_resnet1_oracle_tensor_available': False}
    if PREV_RESNET1_ORACLE.exists():
        ...
        previous_resnet1_compare = {
            'previous_downblock1_resnet1_oracle_tensor_available': True,
    ...
    metadata = {
        ...
        'path_b_entry_downblock0_downblock1_resnet0_attention0_resnet1_attention1': {'status': 'PASS', 'sample_shape': sample_shape, 'resnet1_shape': list(resnet1_out.shape), 'output_shape': list(entry_official.shape)},
        'previous_downblock1_resnet1_compare': previous_resnet1_compare,
        ...
        'saved_tensors': saved,
        'markers': {
            'TADSR_UNET_DOWNBLOCK1_ATTENTION1_ORACLE_TENSORS': 'PASS',
            'TADSR_UNET_DOWNBLOCK1_ATTENTION1_EFFECTIVE_WEIGHTS': 'PASS',
        },
        'not_in_scope': ['full down_blocks.1', 'down_blocks.1.downsamplers.0', 'down_blocks.2', 'mid_block', 'up_blocks', 'full UNet forward', 'full TADSR inference'],
    }
    META_JSON.write_text(json.dumps(metadata, indent=2, default=str), encoding='utf-8')
    lines = [
        '# TADSR UNet down_blocks.1.attentions.1 Oracle Export',
        '',
        'TADSR_UNET_DOWNBLOCK1_ATTENTION1_ORACLE_TENSORS: PASS',
        'TADSR_UNET_DOWNBLOCK1_ATTENTION1_EFFECTIVE_WEIGHTS: PASS',
        '',
        ...
        f"Path B entry->downblock0->downblock1.resnet0->attention0->resnet1->attention1 output shape: `{metadata['path_b_entry_downblock0_downblock1_resnet0_attention0_resnet1_attention1']['output_shape']}`",
        f"Synthetic manual/official max abs error: `{metadata['manual_vs_official_validation']['synthetic_manual_vs_official_max_abs_error']}`",
        f"Entry manual/official max abs error: `{metadata['manual_vs_official_validation']['entry_manual_vs_official_max_abs_error']}`",
        '',
        'This export stops before down_blocks.1.downsamplers.0, down_blocks.2 and full UNet.',
    ]
    SUMMARY_TXT.write_text('\n'.join(lines) + '\n', encoding='utf-8')
    print('TADSR_UNET_DOWNBLOCK1_ATTENTION1_ORACLE_TENSORS: PASS')
    print('TADSR_UNET_DOWNBLOCK1_ATTENTION1_EFFECTIVE_WEIGHTS: PASS')
