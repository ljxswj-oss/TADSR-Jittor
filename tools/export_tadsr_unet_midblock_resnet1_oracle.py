...
ORACLE_DIR = OUT_DIR / 'oracle_tensors_unet_midblock_resnet1'
META_JSON = ORACLE_DIR / 'unet_midblock_resnet1_oracle_metadata.json'
SUMMARY_TXT = ORACLE_DIR / 'oracle_summary.txt'
EFFECTIVE_WEIGHTS = OUT_DIR / 'converted_unet_midblock_resnet1_effective_weights.npz'
PREV_ATTENTION0_ORACLE = OUT_DIR / 'oracle_tensors_unet_midblock_attention0' / 'entry_midblock_attention0_output.npy'
...
def extract_midblock_output(out):
    if isinstance(out, tuple):
        return out[0], {'return_type': 'tuple', 'output_states_status': 'UNKNOWN_TUPLE', 'tuple_len': len(out)}
...
def main() -> int:
    ...
    with torch.no_grad():
        ...
        mid0 = resnet0(block3_hidden, time_embedding)
        att0 = attention0(mid0, encoder_hidden_states=encoder_hidden_states, return_dict=False)[0]
        entry_inter = resnet_manual(resnet1, att0, time_embedding)
        entry_official = resnet1(att0, time_embedding)
        local_mid_raw = mid(
            block3_hidden,
            temb=time_embedding,
        ...
        local_mid_output, local_mid_return = extract_midblock_output(local_mid_raw)
        x_shape = list(att0.shape)
    ...
    saved = {}
    save_tensor(saved, 'synthetic_midblock_resnet1_input', synthetic_x)
    save_tensor(saved, 'synthetic_midblock_resnet1_temb', synthetic_temb)
    for k, v in synthetic_inter.items():
        save_tensor(saved, f'synthetic_midblock_resnet1_{k}_output' if k != 'output' else 'synthetic_midblock_resnet1_manual_output', v)
    save_tensor(saved, 'synthetic_midblock_resnet1_output', synthetic_official)
    ...
    save_tensor(saved, 'entry_midblock_attention0_output', att0)
    save_tensor(saved, 'entry_midblock_resnet1_input', att0)
    save_tensor(saved, 'entry_midblock_resnet1_temb', time_embedding)
    for k, v in entry_inter.items():
        save_tensor(saved, f'entry_midblock_resnet1_{k}_output' if k != 'output' else 'entry_midblock_resnet1_manual_output', v)
    save_tensor(saved, 'entry_midblock_resnet1_output', entry_official)
    save_tensor(saved, 'local_midblock_hidden_states_output', local_mid_output)

    effective = {}
    effective_meta = {}
    prefix = 'mid_block_resnets_1'
    ...
    local_diff = (entry_inter['output'] - local_mid_output).abs()
    previous_attention0_compare = {'previous_midblock_attention0_oracle_tensor_available': False}
    if PREV_ATTENTION0_ORACLE.exists():
        prev = np.load(PREV_ATTENTION0_ORACLE)
        curr = to_np(att0)
        previous_attention0_compare = {
            'previous_midblock_attention0_oracle_tensor_available': True,
    ...
    metadata = {
        ...
        'midblock_resnet0_output_shape': list(mid0.shape),
        'midblock_attention0_output_shape': list(att0.shape),
        'midblock_resnet1_input_shape': list(att0.shape),
        'midblock_resnet1_output_shape': list(entry_official.shape),
        'local_midblock_hidden_states_output_shape': list(local_mid_output.shape),
        'local_midblock_output_states_status': local_mid_return.get('output_states_status', 'NOT_APPLICABLE'),
        'temb_shape': list(time_embedding.shape),
        'midblock_resnet1_consumes': ['hidden_states', 'temb'],
        'midblock_resnet1_consumes_accumulated_residuals': False,
        'midblock_consumes_accumulated_residuals': False,
        ...
        'manual_vs_official': {
            ...
            'local_midblock_hidden_max_abs_diff': float(local_diff.max().item()) if local_diff.numel() else 0.0,
            'local_midblock_hidden_mean_abs_diff': float(local_diff.mean().item()) if local_diff.numel() else 0.0,
        },
        'previous_midblock_attention0_compare': previous_attention0_compare,
        'markers': {
            'TADSR_UNET_MIDBLOCK_RESNET1_ORACLE_TENSORS': 'PASS',
            'TADSR_UNET_MIDBLOCK_RESNET1_EFFECTIVE_WEIGHTS': 'PASS',
            'TADSR_UNET_MIDBLOCK_LOCAL_FORWARD_ORACLE': 'PASS',
        },
        'scope_boundaries': {
            'ported_this_stage': 'mid_block.resnets.1 and local mid_block hidden output',
    ...
    SUMMARY_TXT.write_text(
        '\n'.join([
            '# TADSR UNet mid_block.resnets.1 oracle export',
            '',
            'TADSR_UNET_MIDBLOCK_RESNET1_ORACLE_TENSORS: PASS',
            'TADSR_UNET_MIDBLOCK_RESNET1_EFFECTIVE_WEIGHTS: PASS',
            'TADSR_UNET_MIDBLOCK_LOCAL_FORWARD_ORACLE: PASS',
            ...
            f'local midblock manual/offical max abs diff: {metadata["manual_vs_official"]["local_midblock_hidden_max_abs_diff"]}',
            f'previous attention0 compare: {previous_attention0_compare}',
        ]) + '\n',
        encoding='utf-8',
    )
