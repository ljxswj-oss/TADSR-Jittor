...
ORACLE_DIR = OUT_DIR / 'oracle_tensors_unet_upblock3_resnet1'
META_JSON = ORACLE_DIR / 'unet_upblock3_resnet1_oracle_metadata.json'
SUMMARY_TXT = ORACLE_DIR / 'oracle_summary.txt'
EFFECTIVE_WEIGHTS = OUT_DIR / 'converted_unet_upblock3_resnet1_effective_weights.npz'
PREV_ATTENTION0_ORACLE = OUT_DIR / 'oracle_tensors_unet_upblock3_attention0' / 'entry_upblock3_attention0_output.npy'
...
def main() -> int:
    ...
    saved = {}
    for name, tensor in [
        ('synthetic_upblock3_resnet1_hidden_input', synthetic_hidden),
        ('synthetic_upblock3_resnet1_res_hidden', synthetic_res),
        ('synthetic_upblock3_resnet1_concat_input', synthetic_concat),
        ('synthetic_upblock3_resnet1_temb', synthetic_temb),
    ]:
        save_tensor(saved, name, tensor)
    save_resnet_intermediates(saved, 'synthetic_upblock3_resnet1', synthetic_inter, synthetic_official)
    ...
    for name, tensor in [
        ...
        ('entry_upblock3_resnet1_hidden_input', attention0_out),
        ('entry_upblock3_resnet1_res_hidden', res1_hidden),
        ('entry_upblock3_resnet1_concat_input', concat1),
        ('entry_upblock3_resnet1_temb', ctx['time_embedding']),
    ]:
        save_tensor(saved, name, tensor)
    save_resnet_intermediates(saved, 'entry_upblock3_resnet1', entry_inter, entry_official)

    effective = {}
    effective_meta = {}
    prefix = 'up_blocks_3_resnets_1'
    ...
    previous_attention0_compare = {'previous_upblock3_attention0_oracle_tensor_available': False}
    if PREV_ATTENTION0_ORACLE.exists():
        prev = np.load(PREV_ATTENTION0_ORACLE)
        curr = to_np(attention0_out)
        previous_attention0_compare = {
            'previous_upblock3_attention0_oracle_tensor_available': True,
    ...
    metadata = {
        ...
        'upblock3_resnet1_residual_input_shape': list(res1_hidden.shape),
        'upblock3_resnet1_concat_input_shape': list(concat1.shape),
        'upblock3_resnet1_output_shape': list(entry_official.shape),
        ...
        'previous_upblock3_attention0_compare': previous_attention0_compare,
        ...
        'markers': {
            'TADSR_UNET_UPBLOCK3_RESNET1_ORACLE_TENSORS': 'PASS',
            'TADSR_UNET_UPBLOCK3_RESNET1_EFFECTIVE_WEIGHTS': 'PASS',
            'TADSR_UNET_UPBLOCK3_RESNET1_RESIDUAL_CONTRACT_ORACLE': 'PASS',
        },
    }
    META_JSON.write_text(json.dumps(metadata, indent=2), encoding='utf-8')
    SUMMARY_TXT.write_text('\n'.join([
        '# TADSR UNet up_blocks.3.resnets.1 PyTorch oracle',
        '',
        *[f'{k}: {v}' for k, v in metadata['markers'].items()],
        '',
        f"residual source: {metadata['upblock3_resnet1_consumed_residual_source']}",
        f"hidden input shape: {metadata['upblock3_resnet1_hidden_input_shape']}",
        f"residual shape: {metadata['upblock3_resnet1_residual_input_shape']}",
        f"concat shape: {metadata['upblock3_resnet1_concat_input_shape']}",
        f"output shape: {metadata['upblock3_resnet1_output_shape']}",
