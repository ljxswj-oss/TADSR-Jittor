...
ORACLE_DIR = OUT_DIR / 'oracle_tensors_unet_upblock1_attention2'
META_JSON = ORACLE_DIR / 'unet_upblock1_attention2_oracle_metadata.json'
SUMMARY_TXT = ORACLE_DIR / 'oracle_summary.txt'
EFFECTIVE_WEIGHTS = OUT_DIR / 'converted_unet_upblock1_attention2_effective_weights.npz'
PREV_UPBLOCK1_RESNET2_ORACLE = OUT_DIR / 'oracle_tensors_unet_upblock1_resnet2' / 'entry_upblock1_resnet2_output.npy'
...
def main() -> int:
    ...
    if not getattr(up1, 'upsamplers', None):
        raise RuntimeError('Official up_blocks.1.upsamplers.0 must exist after attentions.2.')
    ...
    with torch.no_grad():
        ...
        resnet2_out = resnet2(concat2, time_embedding)
    ...
    saved = {}
    for name, tensor in [
        ('synthetic_upblock1_attention2_input', synthetic_hidden),
        ('synthetic_upblock1_attention2_encoder_hidden_states', encoder_hidden_states),
    ]:
        save_tensor(saved, name, tensor)
    for k, v in synthetic_manual.items():
        save_tensor(saved, f'synthetic_upblock1_attention2_{k}', v)
    save_tensor(saved, 'synthetic_upblock1_attention2_official_output', synthetic_official)
    ...
    for k, v in entry_manual.items():
        save_tensor(saved, f'entry_upblock1_attention2_{k}', v)
    save_tensor(saved, 'entry_upblock1_attention2_official_output', entry_official)

    arrays = {}
    emeta = {}
    prefix = 'up_blocks_1_attentions_2'
    ...
    emeta[f'{prefix}_norm'] = {'is_lora_wrapped': False, 'weight_shape': list(attention2.norm.weight.shape), 'bias_shape': list(attention2.norm.bias.shape)}
    ...
    previous_resnet2_compare = {'previous_upblock1_resnet2_oracle_tensor_available': False}
    if PREV_UPBLOCK1_RESNET2_ORACLE.exists():
        prev = np.load(PREV_UPBLOCK1_RESNET2_ORACLE)
        curr = to_np(resnet2_out)
        previous_resnet2_compare = {
            'previous_upblock1_resnet2_oracle_tensor_available': True,
    ...
    metadata = {
        ...
        'upblock1_resnet2_consumed_residual_source': sources[resnet2_consumed_index],
        ...
        'actual_next_module_after_upblock1_attention2': 'up_blocks.1.upsamplers.0',
        ...
        'path_a_isolated_attention2': {'status': 'PASS', 'input_shape': h_shape, 'encoder_shape': encoder_shape, 'output_shape': list(synthetic_official.shape)},
        'path_b_entry_downblocks_midblock_upblock0_upblock1_resnet0_attention0_resnet1_attention1_resnet2_attention2': {'status': 'PASS', 'sample_shape': sample_shape, 'input_shape': list(resnet2_out.shape), 'output_shape': list(entry_official.shape)},
        'previous_upblock1_resnet2_compare': previous_resnet2_compare,
        ...
        'markers': {
            ...
            'TADSR_UNET_UPBLOCK1_ATTENTION2_RESIDUAL_CONTRACT_ORACLE': 'PASS',
        },
        'scope_boundaries': {
            'ported_this_stage': 'up_blocks.1.attentions.2 only, after up_blocks.1.resnets.2',
            'not_ported_this_stage': ['up_blocks.1.upsamplers.0', 'full up_blocks.1', 'up_blocks.2', 'up_blocks.3', 'full UNet forward', 'full TADSR inference', 'generic runtime LoRA'],
        },
    }
    META_JSON.write_text(json.dumps(metadata, indent=2, default=str), encoding='utf-8')
    SUMMARY_TXT.write_text('\n'.join([
        '# TADSR UNet up_blocks.1.attentions.2 PyTorch oracle',
        '',
        *[f'{k}: {v}' for k, v in metadata['markers'].items()],
        '',
        f"input shape: {metadata['path_b_entry_downblocks_midblock_upblock0_upblock1_resnet0_attention0_resnet1_attention1_resnet2_attention2']['input_shape']}",
        f"output shape: {metadata['path_b_entry_downblocks_midblock_upblock0_upblock1_resnet0_attention0_resnet1_attention1_resnet2_attention2']['output_shape']}",
        ...
        'next unopened module: up_blocks.1.upsamplers.0',
