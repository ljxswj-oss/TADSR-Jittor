...
ORACLE_DIR = OUT_DIR / 'oracle_tensors_unet_upblock3_attention2'
META_JSON = ORACLE_DIR / 'unet_upblock3_attention2_oracle_metadata.json'
SUMMARY_TXT = ORACLE_DIR / 'oracle_summary.txt'
EFFECTIVE_WEIGHTS = OUT_DIR / 'converted_unet_upblock3_attention2_effective_weights.npz'
PREV_UPBLOCK3_RESNET2_ORACLE = OUT_DIR / 'oracle_tensors_unet_upblock3_resnet2' / 'entry_upblock3_resnet2_output.npy'
...
def main() -> int:
    ...
    if len(getattr(up3, 'resnets', []) or []) < 3 or len(getattr(up3, 'attentions', []) or []) < 3:
        raise RuntimeError('Official up_blocks.3 must expose resnets.0/1/2 and attentions.0/1/2 for this stage.')
    resnet0 = up3.resnets[0]
    attention0 = up3.attentions[0]
    resnet1 = up3.resnets[1]
    attention1 = up3.attentions[1]
    resnet2 = up3.resnets[2]
    attention2 = up3.attentions[2]
    if len(attention2.transformer_blocks) != 1:
        raise RuntimeError(f'Expected one transformer block, got {len(attention2.transformer_blocks)}')
    ...
    with torch.no_grad():
        ...
        resnet2_out = resnet2(concat2, ctx['time_embedding'])

        entry_manual = manual_transformer_attention(attention2, resnet2_out, encoder_hidden_states)
        entry_official = attention2(resnet2_out, encoder_hidden_states=encoder_hidden_states, return_dict=False)[0]

        h_shape = list(resnet2_out.shape)
        synthetic_hidden = torch.linspace(-1.0, 1.0, steps=int(np.prod(h_shape)), dtype=torch.float32).reshape(*h_shape)
        synthetic_manual = manual_transformer_attention(attention2, synthetic_hidden, encoder_hidden_states)
        synthetic_official = attention2(synthetic_hidden, encoder_hidden_states=encoder_hidden_states, return_dict=False)[0]

    saved = {}
    for name, tensor in [
        ('synthetic_upblock3_attention2_input', synthetic_hidden),
        ('synthetic_upblock3_attention2_encoder_hidden_states', encoder_hidden_states),
    ]:
        save_tensor(saved, name, tensor)
    for key, tensor in synthetic_manual.items():
        save_tensor(saved, f'synthetic_upblock3_attention2_{key}', tensor)
    save_tensor(saved, 'synthetic_upblock3_attention2_official_output', synthetic_official)
    ...
    for name, tensor in [
        ...
        ('entry_upblock3_attention0_output', attention0_out),
        ('entry_upblock3_resnet1_output', resnet1_out),
        ('entry_upblock3_attention1_output', attention1_out),
        ('entry_upblock3_resnet2_hidden_input', attention1_out),
        ('entry_upblock3_resnet2_res_hidden', res2_hidden),
        ('entry_upblock3_resnet2_concat_input', concat2),
        ('entry_upblock3_resnet2_output', resnet2_out),
        ('entry_upblock3_attention2_input', resnet2_out),
    ]:
        save_tensor(saved, name, tensor)
    for key, tensor in entry_manual.items():
        save_tensor(saved, f'entry_upblock3_attention2_{key}', tensor)
    save_tensor(saved, 'entry_upblock3_attention2_official_output', entry_official)

    arrays = {}
    emeta = {}
    prefix = 'up_blocks_3_attentions_2'
    arrays[f'{prefix}_norm_weight'] = attention2.norm.weight.detach().cpu().numpy().astype(np.float32)
    arrays[f'{prefix}_norm_bias'] = attention2.norm.bias.detach().cpu().numpy().astype(np.float32)
    emeta[f'{prefix}_norm'] = {
        'is_lora_wrapped': False,
        'weight_shape': list(attention2.norm.weight.shape),
        'bias_shape': list(attention2.norm.bias.shape),
    }
    module_effective_arrays(f'{prefix}_proj_in', attention2.proj_in, arrays, emeta)
    module_effective_arrays(f'{prefix}_proj_out', attention2.proj_out, arrays, emeta)
    tb = attention2.transformer_blocks[0]
    ...
    previous_resnet2_compare = {'previous_upblock3_resnet2_oracle_tensor_available': False}
    if PREV_UPBLOCK3_RESNET2_ORACLE.exists():
        prev = np.load(PREV_UPBLOCK3_RESNET2_ORACLE)
        curr = to_np(resnet2_out)
        previous_resnet2_compare = {
            'previous_upblock3_resnet2_oracle_tensor_available': True,
    ...
    res2_global_index = len(remaining_external_after_up3_tuple_slice) + len(remaining_after_resnet1_local) - 1
    ...
    metadata = {
        ...
        'upblock3_attention1_output_shape': list(attention1_out.shape),
        'upblock3_attention2_input_shape': list(resnet2_out.shape),
        'upblock3_resnet2_output_shape': list(resnet2_out.shape),
        'upblock3_attention2_output_shape': list(entry_official.shape),
        ...
        'upblock3_resnet0_consumed_residual_source': sources[res0_global_index],
        ...
        'upblock3_resnet1_consumed_residual_source': sources[res1_global_index],
        'upblock3_resnet1_consumed_residual_shape': list(res1_hidden.shape),
        'remaining_residual_count_after_upblock3_resnet1_pop': len(remaining_after_resnet1_local),
        'remaining_residual_shapes_after_upblock3_resnet1_pop': [list(x.shape) for x in remaining_after_resnet1_local],
        'remaining_residual_count_after_upblock3_attention2': len(remaining_after_resnet2_local),
        'upblock3_resnet2_consumed_residual_source': sources[res2_global_index],
        'upblock3_resnet2_consumed_residual_shape': list(res2_hidden.shape),
        'remaining_residual_count_after_upblock3_resnet2_pop': len(remaining_after_resnet2_local),
        'attention2_consumes_accumulated_residuals': False,
        'actual_next_module_after_upblock3_attention2': next_module,
        'attention_config': {
            'prefix': prefix,
            'in_channels': int(attention2.in_channels),
            'inner_dim': int(attention2.num_attention_heads) * int(attention2.attention_head_dim),
            'num_attention_heads': int(attention2.num_attention_heads),
            'attention_head_dim': int(attention2.attention_head_dim),
            'cross_attention_dim': int(unet.config.cross_attention_dim),
            'use_linear_projection': bool(attention2.use_linear_projection),
            'norm_num_groups': int(attention2.norm.num_groups),
            'norm_eps': float(attention2.norm.eps),
        },
        'transformer0_config': {
            'heads': int(attention2.num_attention_heads),
            'head_dim': int(attention2.attention_head_dim),
            'inner_dim': int(attention2.num_attention_heads) * int(attention2.attention_head_dim),
        ...
        'previous_upblock3_resnet2_compare': previous_resnet2_compare,
        ...
        'markers': {
            'TADSR_UNET_UPBLOCK3_ATTENTION2_ORACLE_TENSORS': 'PASS',
            'TADSR_UNET_UPBLOCK3_ATTENTION2_EFFECTIVE_WEIGHTS': 'PASS',
            'TADSR_UNET_UPBLOCK3_PRE_LOCAL_AGGREGATE_ALIGNMENT': 'PASS',
            'TADSR_UNET_UPBLOCK3_ATTENTION2_RESIDUAL_CONTRACT_ORACLE': 'PASS',
        },
    }
    META_JSON.write_text(json.dumps(metadata, indent=2), encoding='utf-8')
    SUMMARY_TXT.write_text('\n'.join([
        '# TADSR UNet up_blocks.3.attentions.2 PyTorch oracle',
        '',
        *[f'{k}: {v}' for k, v in metadata['markers'].items()],
        '',
        f"attention input shape: {list(resnet1_out.shape)}",
        ...
        f"next unopened module: {metadata['actual_next_module_after_upblock3_attention2']}",
    ]) + '\n', encoding='utf-8')
