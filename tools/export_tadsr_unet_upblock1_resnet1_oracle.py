...
ORACLE_DIR = OUT_DIR / 'oracle_tensors_unet_upblock1_resnet1'
META_JSON = ORACLE_DIR / 'unet_upblock1_resnet1_oracle_metadata.json'
SUMMARY_TXT = ORACLE_DIR / 'oracle_summary.txt'
EFFECTIVE_WEIGHTS = OUT_DIR / 'converted_unet_upblock1_resnet1_effective_weights.npz'
PREV_UPBLOCK1_ATTENTION0_ORACLE = OUT_DIR / 'oracle_tensors_unet_upblock1_attention0' / 'entry_upblock1_attention0_output.npy'
...
def main() -> int:
    ...
    if len(up0.resnets) < 3 or not getattr(up0, 'upsamplers', None):
        raise RuntimeError('Official up_blocks.0 must be locally complete before exporting up_blocks.1.resnets.1.')
    ...
    saved = {}
    for name, tensor in [
        ('synthetic_upblock1_resnet1_hidden_input', synthetic_h),
        ('synthetic_upblock1_resnet1_res_hidden', synthetic_res),
        ('synthetic_upblock1_resnet1_concat_input', synthetic_concat),
        ('synthetic_upblock1_resnet1_temb', synthetic_temb),
    ]:
        save_tensor(saved, name, tensor)
    save_local_resnet_intermediates(saved, 'synthetic_upblock1_resnet1', synthetic_inter, synthetic_official)
    for name, tensor in [
    ...
    for name, tensor in [
        ...
        ('entry_upblock1_resnet1_temb', time_embedding),
    ]:
        save_tensor(saved, name, tensor)
    save_local_resnet_intermediates(saved, 'entry_upblock1_resnet1', entry_inter, entry_official)

    effective = {}
    effective_meta = {}
    prefix = 'up_blocks_1_resnets_1'
    ...
    previous_attention0_compare = {'previous_upblock1_attention0_oracle_tensor_available': False}
    if PREV_UPBLOCK1_ATTENTION0_ORACLE.exists():
        prev = np.load(PREV_UPBLOCK1_ATTENTION0_ORACLE)
        curr = to_np(attention0_out)
        previous_attention0_compare = {
            'previous_upblock1_attention0_oracle_tensor_available': True,
    ...
    metadata = {
        ...
        'upblock1_resnet1_output_shape': list(entry_official.shape),
        'concat_axis': 1,
        ...
        'previous_upblock1_attention0_compare': previous_attention0_compare,
        'actual_next_module_after_upblock1_resnet1': 'up_blocks.1.attentions.1' if len(getattr(up1, 'attentions', [])) > 1 else 'up_blocks.1.resnets.2',
        'markers': {
            'TADSR_UNET_UPBLOCK1_RESNET1_ORACLE_TENSORS': 'PASS',
            'TADSR_UNET_UPBLOCK1_RESNET1_EFFECTIVE_WEIGHTS': 'PASS',
            'TADSR_UNET_UPBLOCK1_RESNET1_RESIDUAL_CONTRACT_ORACLE': 'PASS',
        },
        'scope_boundaries': {
            'ported_this_stage': 'up_blocks.1.resnets.1 only, including residual consumption after up_blocks.1.attentions.0',
    ...
    SUMMARY_TXT.write_text('\n'.join([
        '# TADSR UNet up_blocks.1.resnets.1 PyTorch oracle',
        '',
        *[f'{k}: {v}' for k, v in metadata['markers'].items()],
        '',
        f"residual source: {metadata['upblock1_resnet1_consumed_residual_source']}",
        f"hidden input shape: {metadata['upblock1_resnet1_hidden_input_shape']}",
        f"concat shape: {metadata['upblock1_resnet1_concat_input_shape']}",
        f"output shape: {metadata['upblock1_resnet1_output_shape']}",
        ...
        f"next unopened module: {metadata['actual_next_module_after_upblock1_resnet1']}",
    ]) + '\n', encoding='utf-8')
